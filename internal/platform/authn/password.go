...
func (Passwords) Hash(password string) (string, error) {
	...
	return fmt.Sprintf("$argon2id$v=%d$m=%d,t=%d,p=%d$%s$%s", argon2.Version, argonMemory, argonIterations, argonParallelism, base64.RawStdEncoding.EncodeToString(salt), base64.RawStdEncoding.EncodeToString(digest)), nil
}

func (Passwords) Verify(encoded, password string) bool {
...
func parsePasswordHash(encoded string) (hashParameters, []byte, []byte, error) {
	...
	if err != nil || parallelism > 16 || memory > 256*1024 || iterations > 10 {
