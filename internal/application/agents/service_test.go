...
func TestPublishPersonaPinsOwnedResultSchemaAndCapabilityAllowlist(t *testing.T) {
	...
	policy := PersonaPolicyDraft{Complexity: agentdomain.PersonaComplexityBalanced, MaximumInputTokens: 100000, MaximumOutputTokens: 4000, MaximumCostMicros: 100000, MaximumToolSteps: 1, CitationPolicy: "best_effort", ActionPolicy: "propose", ActionCapabilities: []string{"finance.entry.post", "marketing.release.activate"}, Tools: []agentdomain.ToolGrant{
	...
	policy.Tools[1].Capability = "finance.entry.draft"
	policy.ActionCapabilities[0] = "email.send"
	if _, _, err := service.PublishPersona(context.Background(), PublishPersonaCommand{Actor: access.Actor{UserID: testUser}, AccountID: testAccount, BoardroomID: testBoardroom, PersonaID: testPersona, VersionID: testVersion, Name: "Operations Lead", Role: "Operations", SystemInstructions: "Coordinate operational work and report evidence clearly.", Policy: policy}); !errors.Is(err, ErrInvalidCommand) {
		t.Fatalf("unregistered action capability=%v", err)
	}
}
