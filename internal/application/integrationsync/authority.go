...
// CurrentAuthority rechecks placement plus the two packages a Drive capture
// crosses. Marketing authority is deliberately unrelated to this read path.
...
func (authority *CurrentAuthority) AuthorizeAccount(ctx context.Context, accountID ids.AccountID) error {
	...
	if state.Account.ID != accountID || state.Entitlements.AccountID != accountID || state.Account.EntitlementVersion == 0 ||
		state.Account.EntitlementVersion != state.Entitlements.Version || state.Account.PlacementGeneration == 0 || state.Account.CellID != authority.cellID {
	...
	for _, code := range []catalog.PackageCode{catalog.PackageKnowledge, catalog.PackageIntegrations} {
