...
func TestServiceRunsAuthorizedVersionBoundLifecycle(t *testing.T) {
	...
	digest := sha256.Sum256([]byte("canonical-plan"))
	assessment, err = service.SubmitPlan(ctx, SubmitPlanCommand{AdvanceCommand: AdvanceCommand{Actor: actor, AccountID: accountID, AssessmentID: assessmentID, ExpectedVersion: assessment.Version, CorrelationID: correlation(9)}, PlanID: planID, ContentSHA256: digest})
	if err != nil {
		t.Fatal(err)
	}
	clock.advance()
