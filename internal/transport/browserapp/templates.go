    ...
    <div class="content">
      ...
      {{if .CanManageMembers}}<section class="team panel" id="settings">
        ...
        <div class="member-list">{{range .Members}}<article>
          <div class="member-identity"><strong>{{.DisplayName}}{{if .IsSelf}} <em>You</em>{{end}}</strong><small>{{.Email}}</small><span>{{.Role}} · version {{.Version}}</span></div>
        ...
        <p class="team-footnote">Role changes, removals, and ownership transfer require a passkey confirmation from the acting identity. Ownership is transferred atomically and every mutation records an immutable reason.</p>
      </section>{{end}}
