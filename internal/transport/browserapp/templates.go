...
<link rel="stylesheet" href="/assets/spyglass.css?v=4">{{if .Script}}<script src="{{.Script}}?v=3" defer></script>{{end}}{{if .PrivacyControls}}<script src="/assets/privacy-analytics.js?v=3" defer></script>{{end}}</head><body><a class="skip-link" href="#main-content">Skip to main content</a>{{if .PrivacyControls}}{{template "privacy-controls" .}}{{end}}
{{end}}
