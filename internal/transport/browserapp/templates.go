...
<link rel="stylesheet" href="/assets/spyglass.css?v=5">{{if .Script}}<script src="{{.Script}}?v=3" defer></script>{{end}}{{if .PrivacyControls}}<script src="/assets/privacy-analytics.js?v=4" defer></script>{{end}}</head><body><a class="skip-link" href="#main-content">Skip to main content</a>{{if .PrivacyControls}}{{template "privacy-controls" .}}{{end}}
{{end}}

{{define "privacy-controls"}}
<section id="privacy-consent" class="privacy-consent" aria-labelledby="privacy-title" hidden><h2 id="privacy-title">Optional analytics</h2><p>Necessary storage keeps signup secure. Optional, content-free analytics improves onboarding and never changes access, checkout, or Affiliate attribution.</p><div id="privacy-options" class="privacy-options" hidden><label><span><strong>Analytics</strong><small>Content-free journey events</small></span><input id="privacy-analytics" type="checkbox"></label><p><strong>Marketing tracking is not used.</strong> No marketing purpose, processor, cookie or destination is configured, so Spyglass does not ask you to consent to one.</p><button type="button" data-privacy-save>Save preferences</button></div><div class="privacy-actions"><button class="secondary" type="button" data-privacy-accept>Accept analytics</button><button class="secondary" type="button" data-privacy-reject>Reject non-essential</button><button class="quiet" type="button" data-privacy-manage>Manage preferences</button></div><p id="privacy-error" class="privacy-error" role="alert"></p></section><button id="privacy-reopen" class="privacy-reopen" type="button" hidden>Privacy choices</button>{{if .AnalyticsEvent}}<span id="analytics-marker" hidden data-event="{{.AnalyticsEvent}}" data-event-id="{{.AnalyticsEventID}}" data-occurred-at="{{.AnalyticsOccurredAt}}"{{if .AnalyticsEventSecond}} data-event-second="{{.AnalyticsEventSecond}}" data-event-second-id="{{.AnalyticsEventSecondID}}"{{end}}{{if .OfferCode}} data-offer="{{.OfferCode}}"{{end}}{{if .AnalyticsMethod}} data-method="{{.AnalyticsMethod}}"{{end}}></span>{{end}}
{{end}}
...
{{define "login"}}
{{template "head" .}}<main class="auth" id="main-content" tabindex="-1"><section class="auth-story">{{template "brand" .}}<div><h1>Welcome to<br><em>Spyglass.</em></h1><p>Sign in to manage your tasks, business records and AI agents.</p></div></section><section class="auth-panel"><form method="post" action="/login"><h2>Sign in to Spyglass</h2>{{template "alert" .}}{{if .GoogleConfigured}}<a class="google-button" href="/auth/google{{if .ReturnTo}}?return_to={{urlquery .ReturnTo}}{{end}}"><span aria-hidden="true">G</span>Continue with Google</a><div class="auth-divider"><span>or use your Spyglass login</span></div>{{end}}<input type="hidden" name="return_to" value="{{.ReturnTo}}"><label>Email address<input type="email" name="email" value="{{.Email}}" autocomplete="email" required></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><p class="form-note"><a href="/forgot-password{{if .ReturnTo}}?return_to={{urlquery .ReturnTo}}{{end}}">Forgot your password?</a></p><button type="submit">Sign in <span>→</span></button>{{if .PasskeysConfigured}}<div class="auth-divider"><span>or</span></div><button class="passkey-button secondary" id="passkey-login" type="button" data-return-to="{{.ReturnTo}}">Sign in with a passkey <span>◇</span></button><p class="passkey-status" id="passkey-status" role="status"></p>{{end}}<p class="form-note">New to Infinite Ocean? <a href="/signup{{if .ReturnTo}}?return_to={{urlquery .ReturnTo}}{{end}}">Create an Account</a>.</p></form></section></main></body></html>
{{end}}

{{define "forgot"}}
{{template "head" .}}<main class="auth" id="main-content" tabindex="-1"><section class="auth-story">{{template "brand" .}}<div><h1>Recover your account</h1><p>Your Infinite Ocean identity spans Spyglass Accounts. Recover the identity once; no Account data or Membership is recreated.</p></div></section><section class="auth-panel"><form method="post" action="/forgot-password"><h2>Find your identity</h2>{{template "alert" .}}{{if .DevelopmentToken}}<div class="dev-link"><strong>Development recovery</strong><a href="/reset-password?token={{.DevelopmentToken}}{{if .ReturnTo}}&return_to={{urlquery .ReturnTo}}{{end}}">Set a new password</a></div>{{else}}<label>Email address<input type="email" name="email" value="{{.Email}}" autocomplete="email" required></label>{{if .ReturnTo}}<input type="hidden" name="return_to" value="{{.ReturnTo}}">{{end}}<button type="submit">Send recovery link <span>→</span></button>{{end}}<p class="form-note"><a href="/login{{if .ReturnTo}}?return_to={{urlquery .ReturnTo}}{{end}}">Return to sign in</a>.</p></form></section></main></body></html>
{{end}}

{{define "reset"}}
{{template "head" .}}<main class="auth" id="main-content" tabindex="-1"><section class="auth-story">{{template "brand" .}}<div><h1>Reset your password</h1><p>Completing recovery invalidates the link, changes the password, and signs the identity out everywhere.</p></div></section><section class="auth-panel"><form method="post" action="/reset-password"><h2>Reset your password</h2>{{template "alert" .}}<input type="hidden" name="token" value="{{.Token}}">{{if .ReturnTo}}<input type="hidden" name="return_to" value="{{.ReturnTo}}">{{end}}<label>New password<input type="password" name="password" autocomplete="new-password" minlength="12" aria-describedby="password-requirements" required><small id="password-requirements">Use at least 12 characters.</small></label><button type="submit">Update password <span>→</span></button><p class="form-note"><a href="/login{{if .ReturnTo}}?return_to={{urlquery .ReturnTo}}{{end}}">Return to sign in</a>.</p></form></section></main></body></html>
{{end}}

{{define "signup"}}
{{template "head" .}}<main class="auth" id="main-content" tabindex="-1"><section class="auth-story">{{template "brand" .}}<div><h1>Create your<br><em>Spyglass account.</em></h1><p>Enter your business details, then review the plan and price before paying.</p></div></section><section class="auth-panel"><form method="post" action="/signup"><h2>Create your Account</h2>{{template "alert" .}}{{if .DevelopmentToken}}<div class="dev-link"><strong>Development verification</strong><a href="/verify?token={{.DevelopmentToken}}{{if .OfferCode}}&offer={{.OfferCode}}{{end}}{{if .ReturnTo}}&return_to={{urlquery .ReturnTo}}{{end}}">Continue to password setup</a></div>{{else}}<label>Business name<input name="account_name" value="{{.AccountName}}" autocomplete="organization" required></label>{{if .GoogleConfigured}}<button class="google-button" type="submit" formaction="/auth/google/signup" formmethod="post" formnovalidate><span aria-hidden="true">G</span>Sign up with Google</button><div class="auth-divider"><span>or use your work email</span></div>{{end}}<label>Your name<input name="name" value="{{.Name}}" autocomplete="name" required></label><label>Work email<input type="email" name="email" value="{{.Email}}" autocomplete="email" required></label><input type="hidden" name="region" value="us-east">{{if .OfferCode}}<input type="hidden" name="offer_code" value="{{.OfferCode}}">{{end}}{{if .ReturnTo}}<input type="hidden" name="return_to" value="{{.ReturnTo}}">{{end}}<button type="submit">Continue <span>→</span></button>{{end}}<p class="form-note">Already registered? <a href="/login{{if .ReturnTo}}?return_to={{urlquery .ReturnTo}}{{end}}">Sign in</a>.</p></form></section></main></body></html>
{{end}}

{{define "verify"}}
{{template "head" .}}<main class="auth" id="main-content" tabindex="-1"><section class="auth-story">{{template "brand" .}}<div><h1>Set your<br><em>password.</em></h1><p>Your password protects your Infinite Ocean identity across every Account you are invited to join.</p></div></section><section class="auth-panel"><form method="post" action="/verify"><h2>Choose your password</h2>{{template "alert" .}}<input type="hidden" name="token" value="{{.Token}}">{{if .OfferCode}}<input type="hidden" name="offer_code" value="{{.OfferCode}}">{{end}}{{if .ReturnTo}}<input type="hidden" name="return_to" value="{{.ReturnTo}}">{{end}}<label>Password<input type="password" name="password" autocomplete="new-password" minlength="12" aria-describedby="password-requirements" required><small id="password-requirements">Use at least 12 characters.</small></label><button type="submit">Create identity and Account <span>→</span></button></form></section></main></body></html>
{{end}}

{{define "contact-verify"}}
{{template "head" .}}<main class="auth" id="main-content" tabindex="-1"><section class="auth-story">{{template "brand" .}}<div><h1>Confirm your<br><em>new email.</em></h1><p>Confirming the new mailbox changes the login for every Spyglass Account and closes every existing session.</p></div></section><section class="auth-panel"><form method="post" action="/contact-change/verify"><h2>Verify the new email</h2>{{template "alert" .}}<input type="hidden" name="token" value="{{.Token}}"><p class="form-note">This changes only your Infinite Ocean identity email. Account Memberships, roles, and package access stay intact.</p><button type="submit" {{if not .Token}}disabled{{end}}>Change identity email <span>→</span></button><p class="form-note"><a href="/login">Return to sign in</a>.</p></form></section></main></body></html>
{{end}}

{{define "accept"}}
{{template "head" .}}<main class="auth" id="main-content" tabindex="-1"><section class="auth-story">{{template "brand" .}}<div><h1>Join your<br><em>team.</em></h1><p>You can use your current login to access this team.</p></div></section><section class="auth-panel"><form method="post" action="/invitations/accept"><h2>Accept invitation</h2>{{template "alert" .}}<input type="hidden" name="token" value="{{.Token}}"><button type="submit">Join this Account <span>→</span></button><p class="form-note"><a href="/app">Return to Spyglass</a></p></form></section></main></body></html>
{{end}}
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content">
      ...
      {{if .Selected}}
      {{if .OwnerEnrollmentRequired}}<section class="owner-security-gate"><div><h1>Secure your account</h1><p>Ownership carries Membership, billing, and lifecycle authority. Add a phishing-resistant passkey, then save a set of one-time recovery codes. Until both are ready, Spyglass keeps this Account in preview and rejects every Account authorization boundary.</p></div><ol><li><b>01</b><span><strong>Add a passkey</strong><small>Proves user presence and verification for privileged actions.</small></span></li><li><b>02</b><span><strong>Save recovery codes</strong><small>Preserves a governed path if every passkey is lost.</small></span></li></ol><a class="button" href="/app/security">Secure owner identity</a></section>{{end}}
      <section class="hero-panel"><div>{{if .OwnerEnrollmentRequired}}<h2 class="account-state-heading">Your Account exists.<br><em>Owner setup comes next.</em></h2><p>Placement and package previews are available, but Account data and operations remain closed until owner identity setup is complete.</p>{{else}}<h1>Choose your plan</h1><p>Identity, Membership, placement, package access, and the local entitlement snapshot are active for this Account.</p>{{end}}</div><div class="horizon" aria-hidden="true"><i></i><b></b></div></section>
      ...
      <div class="dashboard-grid">
        <section class="panel" id="work"><header><div><h2>What’s moving</h2></div><span>Module boundary reserved</span></header><div class="work-empty"><strong>No operational work has been created.</strong><p>The production Work module will populate this view through Account-scoped queries; this shell does not invent customer activity.</p></div></section>
      ...
      <section class="packages"><header><div><h2>Your operating surface</h2></div><span>{{.Selected.AccountType}} Account</span></header><div>{{range .Catalog.Packages}}<article id="{{.Code}}"><b>·</b><div><strong>{{.Name}}</strong><small>{{.Description}}</small></div><em>{{with index $.PackageModes .Code}}{{.}}{{else}}locked{{end}}</em></article>{{end}}</div></section>
      <section class="billing panel" id="billing">
        <header><div><h2>Choose the operating surface that earns its place</h2></div><span>{{.BillingSynced}}</span></header>
      ...
      {{if .CanManageMembers}}<section class="team panel" id="settings">
        <header><div><h2>People with access</h2></div><span>Your role: {{.Selected.Role}}</span></header>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content work-content">
      ...
      <section class="work-locked panel"><div><h1>Work</h1><p>Work is not included in this Account's current package set. Upgrade to Team or Operating to coordinate accountable work across people and agents.</p><a href="/app#billing">Review Account plans →</a></div><div class="work-locked-map" aria-hidden="true"><i></i><i></i><i></i><b></b></div></section>
      {{else}}
      <section class="work-heading"><div><h1>Work</h1><p>One Account-scoped queue for human commitments, agent activity, and operational follow-through.</p></div><div class="work-heading-actions"><span class="work-mode">{{if .WorkReadOnly}}READ-ONLY ACCESS{{else}}PACKAGE ENABLED{{end}}</span>{{if not .WorkReadOnly}}<button class="work-new" id="work-create-open" type="button">＋ New work</button>{{end}}</div></section>
      <section class="work-app" id="work-app" data-account-id="{{.Selected.AccountID}}" data-read-only="{{.WorkReadOnly}}" aria-busy="true">
        ...
        <div class="work-layout">
          <section class="work-queue panel">
            <header><div><h2>Operational work</h2></div><span id="work-result-count">Loading</span></header>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content knowledge-content">
      ...
      <section class="work-locked panel"><div><h1>Knowledge</h1><p>Knowledge is not included in this Account's current package set.</p><a href="/app#billing">Review Account plans →</a></div></section>
      {{else}}
      <section class="work-heading"><div><h1>Knowledge</h1><p>Review source-attributed claims and inspect the current accepted fact projection. Agent output remains a proposal until a person decides.</p></div><span class="work-mode">{{if .KnowledgeReadOnly}}READ-ONLY ACCESS{{else}}PACKAGE ENABLED{{end}}</span></section>
      <section class="knowledge-app" id="knowledge-app" data-account-id="{{.Selected.AccountID}}" data-read-only="{{.KnowledgeReadOnly}}" aria-busy="true">
        ...
        <div class="knowledge-layout">
          <section class="panel knowledge-queue"><header><div><h2>Proposed claims</h2></div><button class="secondary" id="knowledge-refresh" type="button">Refresh</button></header><p id="knowledge-claims-status" role="status">Loading claims…</p><div id="knowledge-claims" class="knowledge-list"></div></section>
          <aside class="panel knowledge-detail" id="knowledge-detail" tabindex="-1"><h2>Select a claim</h2><p>Inspect its exact canonical value and citations before deciding.</p></aside>
        </div>
        <section class="panel knowledge-facts"><header><div><h2>Current facts</h2></div><span id="knowledge-facts-count">Loading</span></header><div id="knowledge-facts" class="knowledge-list"></div></section>
      </section>
      {{end}}
    </div>
  </main>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content marketing-content">
      ...
      <section class="work-locked panel"><div><h1>Marketing</h1><p>Marketing becomes available when this Account has the package. Drafts stay provider-neutral and no activation sends externally.</p><a href="/app#billing">Review Account plans →</a></div></section>
      {{else}}
      <section class="marketing-heading"><div><h1>Marketing</h1><p>Freeze creative revisions and approve one exact release before any Integration can deliver it.</p></div><div class="marketing-toolbar"><span class="work-mode">{{if .MarketingReadOnly}}READ-ONLY ACCESS{{else}}PACKAGE ENABLED{{end}}</span><button id="marketing-refresh" type="button">Refresh</button>{{if not .MarketingReadOnly}}<button id="marketing-new-campaign" type="button">New campaign</button>{{end}}</div></section>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content finance-content">
      ...
      <section class="work-locked panel"><div><h1>Finance</h1><p>Finance is not included in this Account's current package set.</p><a href="/app#billing">Review Account plans →</a></div></section>
      {{else}}
      <section class="finance-heading"><div><h1>Finance</h1><p>Draft balanced journal entries, post them deliberately, and reconcile evidence without bypassing Account authority.</p></div><span class="work-mode">{{if .FinanceReadOnly}}READ-ONLY ACCESS{{else}}PACKAGE ENABLED{{end}}</span></section>
      <section class="finance-app" id="finance-app" data-account-id="{{.Selected.AccountID}}" data-read-only="{{.FinanceReadOnly}}" aria-busy="true">
        ...
        <section class="finance-tab-panel" id="finance-chart-panel" role="tabpanel" aria-labelledby="finance-chart-tab">
          <div class="finance-layout">
            <section class="panel finance-list-panel"><header><div><h2>Posting accounts</h2></div>{{if not .FinanceReadOnly}}<button class="secondary" id="finance-new-account" type="button">Add account</button>{{end}}</header><p class="finance-status" id="finance-accounts-status" role="status">Choose a ledger.</p><div class="finance-list" id="finance-accounts"></div></section>
            <aside class="panel finance-detail" id="finance-account-detail" tabindex="-1" aria-live="polite"><h2>Select an account</h2><p>Inspect its normal balance, posting state, and current ledger balance.</p></aside>
          </div>
        </section>
        <section class="finance-tab-panel" id="finance-journal-panel" role="tabpanel" aria-labelledby="finance-journal-tab" hidden>
          <div class="finance-layout">
            <section class="panel finance-list-panel"><header><div><h2>Entries</h2></div>{{if not .FinanceReadOnly}}<button class="secondary" id="finance-new-entry" type="button">Draft entry</button>{{end}}</header><div class="finance-filters"><label><span class="sr-only">Entry state</span><select id="finance-entry-state"><option value="">All states</option><option value="draft">Draft</option><option value="posted">Posted</option><option value="reversed">Reversed</option></select></label></div><p class="finance-status" id="finance-entries-status" role="status">Choose a ledger.</p><div class="finance-list" id="finance-entries"></div></section>
            <aside class="panel finance-detail" id="finance-entry-detail" tabindex="-1" aria-live="polite"><h2>Select an entry</h2><p>Inspect balanced lines, provenance, evidence, and posting state.</p></aside>
          </div>
        </section>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content attention-content">
      ...
      <section class="work-locked panel"><div><h1>Your Turn</h1><p>Your Turn becomes available with the Work or Agents package. It collects only questions and decisions this identity is authorized to inspect.</p><a href="/app#billing">Review Account plans →</a></div><div class="work-locked-map" aria-hidden="true"><i></i><i></i><i></i><b></b></div></section>
      {{else}}
      <section class="attention-heading"><div><h1>Your Turn</h1><p>Answer missing information, review version-bound Work, and govern consequential Agent actions from one Account-scoped queue.</p></div><button class="secondary" id="attention-refresh" type="button">Refresh queue</button></section>
      <section class="attention-app" id="attention-app" data-account-id="{{.Selected.AccountID}}" data-user-id="{{.ActorUserID}}" data-work-available="{{.WorkAvailable}}" data-work-read-only="{{.WorkReadOnly}}" data-approvals-available="{{.ApprovalsAvailable}}" data-agents-read-only="{{.AgentsReadOnly}}" aria-busy="true">
        ...
        <div class="attention-layout">
          <section class="attention-queue panel" aria-labelledby="attention-queue-title">
            <header><div><h2 id="attention-queue-title">Waiting for you</h2></div><span id="attention-result-count">Loading</span></header>
          ...
          <aside class="attention-detail panel" id="attention-detail" tabindex="-1" aria-live="polite">
            <div class="attention-detail-empty"><h2>Select an item</h2><p>Choose a queue item to inspect the exact question, proposal version, consequential payload, or redacted recovery status before acting.</p></div>
          </aside>
        </div>
      </section>
      {{end}}
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content agents-content">
      ...
      <section class="agents-locked panel"><div><h1>Agents</h1><p>Agents is not included in this Account's current package set. The Operating plan adds versioned specialists, coordinated Boardrooms, and Account-bound runs.</p><a href="/app#billing">Review Account plans →</a></div><div class="agents-locked-orbit" aria-hidden="true"><i></i><i></i><i></i><b>IO</b></div></section>
      {{else}}
      <section class="agents-heading"><div><h1>Agents</h1><p>Bring governed specialists into one Account-scoped conversation. Every run freezes its Persona versions, policy, tools, and entitlement boundary.</p></div><span class="work-mode">{{if .AgentsReadOnly}}READ-ONLY ACCESS{{else}}PACKAGE ENABLED{{end}}</span></section>
      <section class="agents-app" id="agents-app" data-account-id="{{.Selected.AccountID}}" data-read-only="{{.AgentsReadOnly}}" aria-busy="true">
        <aside class="agents-rail panel">
          <header><div><h2>Operating rooms</h2></div><span id="agents-room-count">Loading</span></header>
        ...
        <section class="agents-stage">
          <section class="agents-roster panel">
            <header><div><h2 id="agents-room-name">Select a Boardroom</h2><p id="agents-room-purpose">Choose an operating room to inspect its team and conversations.</p></div><span id="agents-room-policy"></span></header>
          ...
          <section class="agents-conversations panel">
            <header><div><h2>Decision history</h2></div><span id="agents-conversation-count">Select a room</span></header>
          ...
          <section class="agents-transcript panel" id="agents-transcript" hidden>
            <header><div><h2 id="agents-transcript-title"></h2></div><span id="agents-transcript-state"></span></header>
            ...
            {{if not .AgentsReadOnly}}<form id="agents-run-recovery" class="agents-run-recovery" hidden>
              <div><h3 id="agents-recovery-title">Resolve this run</h3><p id="agents-recovery-summary"></p></div>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content schedules-content">
      ...
      <section class="empty"><h1>No Spyglass Accounts yet.</h1><p>Create an Account or accept an invitation to begin.</p><a href="/signup">Create Account</a></section>
      {{else if not .AgentsAvailable}}
      <section class="agents-locked panel"><div><h1>Schedules</h1><p>Schedules use the Agents package because every occurrence enters the same governed Boardroom Run boundary.</p><a href="/app#billing">Review Account plans →</a></div><div class="agents-locked-orbit" aria-hidden="true"><i></i><i></i><i></i><b>IO</b></div></section>
      {{else}}
      <section class="schedules-heading"><div><h1>Schedules</h1><p>Every occurrence creates a fresh, capacity-governed Agent conversation. Timezone, daylight-saving behavior, missed runs, Personas, and context remain explicit.</p></div><span class="work-mode">{{if .AgentsReadOnly}}READ-ONLY ACCESS{{else}}PACKAGE ENABLED{{end}}</span></section>
      <section class="schedules-app" id="schedules-app" data-account-id="{{.Selected.AccountID}}" data-read-only="{{.AgentsReadOnly}}" aria-busy="true">
        <section class="schedules-list panel">
          <header><div><h2>Recurring Agent runs</h2></div><span id="schedules-count">Loading</span></header>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content lifecycle-content">
      {{template "alert" .}}
      <section class="lifecycle-heading"><div><h1>Close account</h1><p>This identity-wide surface remains available even when an Account is frozen. It is the recovery path for Accounts you own.</p></div><a href="/app">Return to active Accounts</a></section>
      <section class="lifecycle-list panel"><header><div><h2>Closure history</h2></div><span>{{len .Closures}} records</span></header>
...
<div class="app-shell">
  ...
  <main class="workspace" id="main-content" tabindex="-1">
    ...
    <div class="content lifecycle-content">
      {{template "alert" .}}
      <section class="lifecycle-heading"><div><h1>Export account data</h1><p>Exports combine the global Account record with its current cell data, then retain one immutable ZIP for seven days.</p></div><a href="/app/security">Confirm with a passkey</a></section>
