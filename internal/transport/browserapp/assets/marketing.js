(() => {
  ...
  const upload = async (path, body) => problem(await fetch(path, { method: "POST", headers: { Accept: "application/json", "Idempotency-Key": crypto.randomUUID() }, body }));
  ...
  document.getElementById("marketing-asset-form")?.addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget; try { await upload(`${base}/campaigns/${activeCampaign.id}/asset-revisions`, new FormData(form)); form.closest("dialog").close(); announce("Immutable asset revision added."); await selectCampaign(activeCampaign.id); } catch (error) { showFormError(form, error); } });
