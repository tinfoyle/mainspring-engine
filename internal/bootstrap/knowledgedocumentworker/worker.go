...
func New(ctx context.Context, config Config, logger *slog.Logger) (*Worker, error) {
	...
	return &Worker{pool: pool, processor: application, objects: objects, scanner: scanner, extractor: extractor, poll: config.PollInterval, logger: logger}, nil
}
...
func (worker *Worker) logFailure(ctx context.Context, processErr error) {
	...
	worker.logger.Error("Knowledge document processing failed", "error", processErr, "ready", stats.Ready, "leased", stats.Leased, "retrying", stats.Retrying, "dead_letter", stats.DeadLetter, "oldest_ready_age_seconds", int64(stats.OldestReadyAge/time.Second))
}
...
func (worker *Worker) Status(ctx context.Context) (any, error) {
	...
	return Status{Pending: stats.Pending, Ready: stats.Ready, Leased: stats.Leased, Retrying: stats.Retrying, Completed: stats.Completed, DeadLetter: stats.DeadLetter, OldestReadyAgeSeconds: int64(stats.OldestReadyAge / time.Second), Processed: worker.processed.Load(), Succeeded: worker.succeeded.Load(), Failures: worker.failures.Load()}, nil
}
