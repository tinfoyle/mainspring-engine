// Package schedulequeueadmin wires the short-lived audited Schedule queue
...
func Run(ctx context.Context, config Config, logger *slog.Logger) error {
	...
	service, err := application.NewService(postgres.NewScheduleQueueAdminRepository(pool), ids.RandomGenerator{})
	if err != nil {
		return err
	}
	...
	case "inspect":
		...
		for _, record := range inspection.DeadLetters {
			logger.Info("Spyglass Schedule queue dead letter", "audit_batch_id", inspection.AuditBatchID, "queue", record.Queue, "account_id", record.AccountID, "schedule_id", record.ScheduleID, "trigger_id", record.TriggerID, "attempt_count", record.AttemptCount, "last_error_code", record.LastErrorCode, "occurrence_at", record.OccurrenceAt, "created_at", record.CreatedAt, "updated_at", record.UpdatedAt)
		}
		logger.Info("Spyglass Schedule queue inspection complete", "audit_batch_id", inspection.AuditBatchID, "queue", config.Queue, "environment", config.Environment, "actor", config.Actor, "count", len(inspection.DeadLetters))
		return nil
	case "requeue":
		...
		logger.Info("Spyglass Schedule queue dead letter requeued", "audit_batch_id", result.AuditBatchID, "queue", record.Queue, "environment", config.Environment, "actor", config.Actor, "account_id", record.AccountID, "schedule_id", record.ScheduleID, "trigger_id", record.TriggerID, "previous_attempt_count", record.AttemptCount, "previous_error_code", record.LastErrorCode, "next_attempt_at", record.NextAttemptAt)
		return nil
	default:
		return fmt.Errorf("unsupported Schedule queue operator action %q", config.Action)
	}
}
...
func validateConfig(config Config, logger *slog.Logger) error {
	if config.DatabaseURL == "" || config.Action == "" || config.Queue == "" || config.Actor == "" || config.Reason == "" || config.Environment == "" || logger == nil {
		return errors.New("Schedule queue operator database, action, queue, actor, reason, environment, and logger are required")
	}
	if config.ConfirmEnvironment == "" || config.ConfirmEnvironment != config.Environment {
		return errors.New("Schedule queue operator environment confirmation must exactly match the target environment")
	}
