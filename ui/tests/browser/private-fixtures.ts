...
export const userID = "20000000-0000-4000-8000-000000000002";
...
export const secondAccountID = "10000000-0000-4000-8000-000000000010";
...
export const terminalPrivacyRightsRequests = [
...
export const baselineID = "11000000-0000-4000-8000-000000000011";
...
export const agentRunID = "16000000-0000-4000-8000-000000000016";
...
export const financeLedger = { id: "22000000-0000-4000-8000-000000000022", account_id: accountID, name: "Operating", code: "OPS", description: "Primary book", currency: "USD", state: "active", version: 3, created_by: { kind: "user", id: userID }, created_at: "2026-08-24T00:00:00Z", updated_at: "2026-08-24T00:00:00Z" };
export const financeSummary = { ...financeLedger, account_count: 2, draft_count: 1, income_minor: 5000, expense_minor: 2000, net_minor: 3000 };
export const financeEntry = { id: "23000000-0000-4000-8000-000000000023", account_id: accountID, ledger_id: financeLedger.id, number: 8, entry_date: "2026-08-24T00:00:00Z", description: "Monthly close", reference: "CLOSE-8", currency: "USD", total_minor: 5000, state: "draft", version: 5, lines: [{ account_id: "cash", memo: "", debit_minor: 5000, credit_minor: 0 }, { account_id: "income", memo: "", debit_minor: 0, credit_minor: 5000 }], evidence: ["30000000-0000-4000-8000-000000000003"], provenance: { source: "manual" }, created_by: { kind: "user", id: userID }, created_at: "2026-08-24T00:00:00Z", updated_at: "2026-08-24T00:00:00Z" };
...
export const integrationHealth = { id: "27000000-0000-4000-8000-000000000027", account_id: accountID, connection_id: integrationConnection.id, connection_revision_id: integrationConnection.current_revision_id, credential_id: integrationConnection.credential_id, state: "healthy", latency_milliseconds: 81, checked_at: "2026-08-24T00:00:00Z" };
export const integrationDetail = { connection: integrationConnection, revision: { id: integrationConnection.current_revision_id, account_id: accountID, connection_id: integrationConnection.id, revision: 2, capabilities: ["web.research"], scope: { https_origin: "https://example.com", path_prefix: "/policy" }, created_by: { user_id: userID }, created_at: "2026-08-24T00:00:00Z" }, latest_health: integrationHealth };
export const integrationExecution = { id: "28000000-0000-4000-8000-000000000028", account_id: accountID, release_id: "29000000-0000-4000-8000-000000000029", release_version: 3, approval_id: "31000000-0000-4000-8000-000000000031", capability: "web.publish", connection_id: integrationConnection.id, connection_revision_id: integrationConnection.current_revision_id, connection_revision: 2, credential_id: integrationConnection.credential_id, credential_generation: 1, payload_sha256: "e".repeat(64), state: "manual_resolution", attempt_count: 1, created_at: "2026-08-24T00:00:00Z", updated_at: "2026-08-24T00:00:00Z" };
export const integrationExecutionDetail = { execution: integrationExecution, attempts: [], resolution: { id: "32000000-0000-4000-8000-000000000032", execution_id: integrationExecution.id, requested_outcome: "succeeded", evidence_sha256: "f".repeat(64), requested_by_user_id: userID, requested_at: "2026-08-24T00:00:00Z", state: "pending" } };
export const marketingCampaign = { id: "33000000-0000-4000-8000-000000000033", account_id: accountID, name: "Launch", objective: "Explain the operating model", audience: "Business owners", channels: ["email", "web"], state: "draft", version: 4, created_by: { kind: "user", id: userID }, provenance: { origin: "human" }, created_at: "2026-08-24T00:00:00Z", updated_at: "2026-08-24T01:00:00Z" };
export const marketingAsset = { id: "34000000-0000-4000-8000-000000000034", account_id: accountID, campaign_id: marketingCampaign.id, asset_id: "35000000-0000-4000-8000-000000000035", revision: 2, kind: "copy", title: "Launch copy", media_type: "text/plain", content_reference: "opaque", content_sha256: "1".repeat(64), content_bytes: 120, created_by: { kind: "user", id: userID }, provenance: { origin: "human" }, created_at: "2026-08-24T00:30:00Z" };
export const marketingRelease = { id: "36000000-0000-4000-8000-000000000036", account_id: accountID, campaign_id: marketingCampaign.id, campaign_version: 4, name: "Launch release", channels: ["email", "web"], asset_revision_ids: [marketingAsset.id], state: "submitted", version: 2, created_by: { kind: "user", id: userID }, provenance: { origin: "human" }, submitted_by: { kind: "user", id: userID }, created_at: "2026-08-24T00:40:00Z", updated_at: "2026-08-24T00:50:00Z" };

export interface SyntheticAPIState {
...
export function fulfillProblem(route: Route, status: number, code: string, detail: string) {
...
export function accountWithPackageModes(mode: "enabled" | "read_only", excluded: ReadonlyArray<string> = []) {
  return {
    ...account,
    entitlements: {
...
export async function overrideSession(page: Page, selectedAccount: ReturnType<typeof accountWithPackageModes>): Promise<void> {
...
export async function installSyntheticAPI(page: Page): Promise<SyntheticAPIState> {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (path === `${accountBase}/schedules/${schedule.id}/history`) {
      await fulfillJSON(route, { items: [] });
      return;
    }
