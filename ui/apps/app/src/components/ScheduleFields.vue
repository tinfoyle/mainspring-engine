...
import type { ScheduleFrequency, ScheduleGapPolicy, ScheduleMissedRunPolicy, ScheduleOverlapPolicy } from "@spyglass/api";
...
const props = defineProps<{ model: Draft }>();
const emit = defineEmits<{ update: [value: Draft] }>();
const fields = reactive<Draft>({ ...props.model, weekdays: [...props.model.weekdays] });
const weekdayOptions = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
let synchronizing = false;

watch(() => props.model, (value) => {
  synchronizing = true;
  Object.assign(fields, value, { weekdays: [...value.weekdays] });
  queueMicrotask(() => { synchronizing = false; });
}, { deep: true });
watch(fields, (value) => {
  if (!synchronizing) emit("update", { ...value, weekdays: [...value.weekdays] });
}, { deep: true });
</script>
