...
import { getPublicCatalog, type AIComplexity, type AIComplexityRate, type AgentPersona, type AgentToolGrantInput, type PublishAgentPersonaRequest } from "@spyglass/api";
...
const tools: ReadonlyArray<ToolDefinition> = [
  ...
  { capability: "finance.ledgers.read", name: "read_finance_ledgers", label: "Read Finance ledgers", description: "List the account’s financial ledgers.", input_schema: closed({}) },
  { capability: "finance.accounts.read", name: "read_finance_accounts", label: "Read chart of accounts", description: "List accounts in a financial ledger.", input_schema: closed({ ledger_id: uuid }, ["ledger_id"]) },
  { capability: "finance.entry.draft", name: "draft_finance_entry", label: "Draft Finance entry", description: "Draft an entry. Posting requires a separate approval.", input_schema: closed({ ledger_id: uuid, run_id: uuid, entry_date: { type: "string", format: "date-time" }, description: { type: "string" }, reference: { type: "string" }, currency: { type: "string", pattern: "^[A-Z]{3}$" }, lines: { type: "array", minItems: 2, items: closed({ account_id: uuid, memo: { type: "string" }, debit_minor: { type: "integer", minimum: 0 }, credit_minor: { type: "integer", minimum: 0 } }, ["account_id", "memo", "debit_minor", "credit_minor"]) }, evidence: { type: "array", items: uuid } }, ["ledger_id", "run_id", "entry_date", "description", "reference", "currency", "lines", "evidence"]) },
  { capability: "marketing.campaigns.read", name: "read_marketing_campaigns", label: "Read Marketing campaigns", description: "List marketing campaigns.", input_schema: closed({ state: { type: "string", enum: ["draft", "active", "paused", "completed", "archived"] } }) },
  { capability: "marketing.asset-revisions.read", name: "read_marketing_creative", label: "Read creative revisions", description: "List saved content versions for a campaign.", input_schema: closed({ campaign_id: uuid, asset_id: uuid }, ["campaign_id"]) },
  { capability: "marketing.releases.read", name: "read_marketing_releases", label: "Read Marketing releases", description: "List campaign releases.", input_schema: closed({ campaign_id: uuid }, ["campaign_id"]) },
  { capability: "marketing.campaign.draft", name: "draft_marketing_campaign", label: "Draft campaign", description: "Create a campaign draft.", input_schema: closed({ run_id: uuid, name: { type: "string" }, objective: { type: "string" }, audience: { type: "string" }, channels: { type: "array", minItems: 1, uniqueItems: true, items: { type: "string", enum: ["email", "web"] } } }, ["run_id", "name", "objective", "audience", "channels"]) },
  { capability: "marketing.asset-revision.draft", name: "draft_marketing_copy", label: "Draft Marketing copy", description: "Save draft text for a campaign.", input_schema: closed({ run_id: uuid, campaign_id: uuid, asset_id: uuid, title: { type: "string" }, content: { type: "string", maxLength: 65536 } }, ["run_id", "campaign_id", "asset_id", "title", "content"]) },
  { capability: "marketing.release.draft", name: "draft_marketing_release", label: "Draft release snapshot", description: "Create a release draft for review.", input_schema: closed({ run_id: uuid, campaign_id: uuid, campaign_version: { type: "integer", minimum: 1 }, name: { type: "string" }, channels: { type: "array", minItems: 1, uniqueItems: true, items: { type: "string", enum: ["email", "web"] } }, asset_revision_ids: { type: "array", minItems: 1, maxItems: 100, uniqueItems: true, items: uuid } }, ["run_id", "campaign_id", "campaign_version", "name", "channels", "asset_revision_ids"]) }
];
const actions = [
...
const maximumInputTokens = ref(props.persona?.policy.maximum_input_tokens ?? 128000); const maximumOutputTokens = ref(props.persona?.policy.maximum_output_tokens ?? 4096); const maximumCostMicros = ref(props.persona?.policy.maximum_cost_micros ?? 500000); const maximumToolSteps = ref(props.persona?.policy.maximum_tool_steps ?? 2);
