      ...
      <template v-else>
        <section class="marketing-workspace">
          ...
          <div class="marketing-stage">
            ...
            <section v-if="selectedCampaign && tab === 'overview'" class="marketing-overview"><article><p class="eyebrow">Creative</p><strong>{{ assets.length }}</strong><span>immutable revision{{ assets.length === 1 ? "" : "s" }}</span><IoButton kind="quiet" @click="tab = 'creative'">Review creative</IoButton></article><article><p class="eyebrow">Releases</p><strong>{{ releases.length }}</strong><span>frozen snapshot{{ releases.length === 1 ? "" : "s" }}</span><IoButton kind="quiet" @click="tab = 'releases'">Review releases</IoButton></article><aside><strong>Activation is not delivery</strong><p>Activation records approved intent. External email or web publication is prepared and evidenced separately in Integrations.</p><RouterLink to="/app/integrations">Open Integrations →</RouterLink></aside></section>
            <section v-else-if="selectedCampaign && tab === 'creative'" class="marketing-collection"><header><div><p class="eyebrow">Immutable creative</p><h2>Asset revisions</h2></div><IoButton v-if="writable && campaignEditable" @click="begin('asset-upload')">Add revision</IoButton></header><ol><li v-for="item in assets" :key="item.id"><div><strong>{{ item.title }}</strong><small>{{ label(item.kind) }} · revision {{ item.revision }} · {{ item.media_type }}</small><span>{{ bytes(item.content_bytes) }} · digest {{ short(item.content_sha256) }}</span><span v-if="item.alternative_text">Alt: {{ item.alternative_text }}</span></div><em>{{ label(item.provenance.origin) }}</em></li></ol><p v-if="assets.length === 0">No creative revisions have been admitted.</p><IoButton v-if="assetCursor" kind="secondary" :disabled="loadingMore" @click="loadMore('assets')">Load more revisions</IoButton></section>
            <section v-else-if="selectedCampaign && tab === 'releases'" class="marketing-releases"><header><div><p class="eyebrow">Approval snapshots</p><h2>Releases</h2></div><IoButton v-if="writable && campaignEditable && assets.length" @click="begin('release-create')">Build release</IoButton></header><div class="marketing-release-layout"><ol><li v-for="item in releases" :key="item.id"><button type="button" :aria-current="selectedRelease?.id === item.id ? 'page' : undefined" @click="navigate(`/app/marketing/releases/${item.id}`)"><span><strong>{{ item.name }}</strong><small>Campaign v{{ item.campaign_version }} · {{ item.asset_revision_ids.length }} creative revision{{ item.asset_revision_ids.length === 1 ? "" : "s" }}</small></span><em>{{ label(item.state) }}</em></button></li></ol><article v-if="selectedRelease" class="marketing-release-detail"><div><p class="eyebrow">Release detail</p><h3>{{ selectedRelease.name }}</h3><span class="state-badge">{{ label(selectedRelease.state) }}</span></div><dl><div><dt>Campaign version</dt><dd>{{ selectedRelease.campaign_version }}{{ releaseCurrent ? " · current" : " · stale" }}</dd></div><div><dt>Channels</dt><dd>{{ selectedRelease.channels.map(label).join(" · ") }}</dd></div><div><dt>Creative</dt><dd>{{ selectedRelease.asset_revision_ids.length }} exact revision{{ selectedRelease.asset_revision_ids.length === 1 ? "" : "s" }}</dd></div><div><dt>Release version</dt><dd>{{ selectedRelease.version }}</dd></div><div v-if="selectedRelease.approval_id"><dt>Approval</dt><dd>{{ short(selectedRelease.approval_id) }}</dd></div></dl><p v-if="selectedRelease.state === 'submitted'">An owner or administrator approves the exact activation request in Your Turn; this screen never accepts an internal approval identifier.</p><p v-if="!releaseCurrent && selectedRelease.state === 'draft'" class="queue-inline-status">Campaign intent changed after this snapshot. Build a new release from the current campaign version.</p><div class="marketing-actions"><IoButton v-if="writable && selectedRelease.state === 'draft' && releaseCurrent" @click="begin('release-submit', selectedRelease)">Submit snapshot</IoButton><RouterLink v-if="selectedRelease.state === 'submitted'" class="io-link-button" to="/app/your-turn">Open Your Turn</RouterLink><IoButton v-if="manageable && ['submitted', 'approved'].includes(selectedRelease.state)" kind="secondary" @click="begin('release-cancel', selectedRelease)">Cancel release</IoButton><IoButton v-if="manageable && releaseActivatable" @click="begin('campaign-activate', selectedRelease)">Activate release</IoButton></div></article><article v-else class="marketing-release-detail"><p class="eyebrow">Release detail</p><h3>Select a release</h3><p>Inspect the frozen campaign version, channels and exact creative revisions.</p></article></div><p v-if="releases.length === 0">No release snapshots have been built.</p><IoButton v-if="releaseCursor" kind="secondary" :disabled="loadingMore" @click="loadMore('releases')">Load more releases</IoButton></section>
          </div>
        </section>
      </template>
    </template>
