...
<template>
  <section class="page affiliate-page">
    ...
    <div v-else-if="errorMessage && !program" class="queue-state queue-state--error" role="alert"><h2>Affiliate details are unavailable</h2><p>{{ errorMessage }}</p><IoButton variant="secondary" @click="load">Try again</IoButton></div>

    <template v-else-if="program?.enrollment">
      <section class="affiliate-code" aria-labelledby="affiliate-code-heading"><div><p class="eyebrow">Your generated code</p><h2 id="affiliate-code-heading">{{ program.enrollment.public_code }}</h2><p>Share this code with a clear disclosure that you may earn recurring value from qualifying purchases. Customers choose whether to apply it in their checkout review.</p></div><IoButton variant="secondary" @click="copyCode">{{ copied ? "Copied" : "Copy code" }}</IoButton></section>
