...
const error = ref(""); const announcement = ref(""); const securityRequired = ref(false); const actionOpen = ref(false); const action = ref<Action>("close");
...
async function submit(): Promise<void> {
  if (saving.value || confirmation.value !== phrase()) return; saving.value = true;
  try {
...
<template>
  <section class="page lifecycle-page">
    ...
    <section v-if="session.selected?.owner_enrollment_required" class="queue-state queue-state--warning"><h2>Secure this owner identity first</h2><p>Add a passkey and recovery codes before using lifecycle authority.</p><a href="/app/security?return_to=%2Fapp%2Faccount-closures">Continue security setup</a></section>
