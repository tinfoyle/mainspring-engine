...
<template>
  <section class="page lifecycle-page">
    ...
    <header class="page-heading"><h1>Close account</h1><p>Request account closure or check an existing request.</p></header>
    ...
    <section v-if="canClose && session.selected" class="lifecycle-request"><div><h2>Close {{ session.selected.display_name }}</h2><p>Closure immediately freezes access for every member and begins a seven-day cooling-off period. Active subscriptions or checkout sessions must be resolved first.</p></div><IoButton @click="begin('close')">Request closure</IoButton></section>
    <section class="lifecycle-history">
      <header><div><h2>Closure history</h2></div><span>{{ closures.length }} records</span></header>
      <div v-if="loading" class="queue-state" role="status">Loading lifecycle history…</div><div v-else-if="closures.length === 0" class="queue-state"><h3>No closure history</h3><p>You can review or cancel a pending closure here.</p></div>
