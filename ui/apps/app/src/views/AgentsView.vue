...
async function saveManager(): Promise<void> {
  const accountID = session.selectedID;
  if (!accountID || !room.value || !managerPersonaID.value || saving.value) return;
...
async function runBoardroom(): Promise<void> {
  const accountID = session.selectedID;
  if (!accountID || !room.value || saving.value || selectedPersonaIDs.value.length === 0) return;
...
async function resolveRun(): Promise<void> {
  const accountID = session.selectedID;
  if (!accountID || !activeRun.value || saving.value) return;
  saving.value = true; roomError.value = "";
  try {
...
<template>
  <section class="page agents-page">
    ...
    <template v-if="!roomID">
      <header class="page-heading page-heading--action"><div><p class="eyebrow">Governed specialist collaboration</p><h1>Agents</h1><p>Convene versioned Personas in Account-scoped Boardrooms while consequential actions remain routed through Your Turn.</p></div><IoButton v-if="writable" @click="createOpen = !createOpen">{{ createOpen ? "Close" : "New Boardroom" }}</IoButton></header>
      ...
      <template v-else>
        <form v-if="createOpen" class="decision-card agents-create" @submit.prevent="createRoom"><h2>Create a Boardroom</h2><label>Name<input v-model="roomDraft.name" minlength="2" maxlength="160" required></label><label>Purpose<textarea v-model="roomDraft.purpose" maxlength="2000" rows="4"></textarea></label><p v-if="error" class="form-error" role="alert">{{ error }}</p><IoButton type="submit" :disabled="saving">{{ saving ? "Creating…" : "Create Boardroom" }}</IoButton></form>
