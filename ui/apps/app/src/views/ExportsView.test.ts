...
describe("Account exports surface", () => {
  it("renders governed history for an owner", async () => { const wrapper = await mountView(); await flushPromises(); expect(wrapper.text()).toContain("Exports"); expect(wrapper.text()).toContain("Cancel request"); await expectNoAxeViolations(wrapper.element); });
