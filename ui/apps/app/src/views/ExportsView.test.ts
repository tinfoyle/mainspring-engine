...
async function mountView() { const router = createRouter({ history: createMemoryHistory(), routes: [{ path: "/app/account-exports", component: ExportsView }] }); await router.push("/app/account-exports"); await router.isReady(); return mount(ExportsView, { global: { plugins: [router] } }); }
