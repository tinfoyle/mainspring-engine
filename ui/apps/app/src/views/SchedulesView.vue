...
function restoreDraft(): void {
  ...
  if (!accountID) return;
  try { Object.assign(draft, JSON.parse(sessionStorage.getItem(draftKey(accountID)) ?? "{}")); } catch { /* Optional tab storage. */ }
}
...
function request(): CreateScheduleRequest {
  return {
    ...
    template: {
      boardroom_id: draft.boardroomID.trim(), mode: draft.mode, persona_ids: splitIDs(draft.personaIDs) ?? [],
...
function fillDraft(value: Schedule): void {
  Object.assign(draft, {
    ...
    personaIDs: value.template.persona_ids.join("\n"), subject: value.template.subject, prompt: value.template.prompt,
  ...
  <section class="page schedules-page">
    ...
    <template v-if="scheduleID">
      ...
      <template v-else-if="detail">
        <header class="detail-heading"><div><p class="eyebrow">Automated Agent run</p><h1>{{ detail.name }}</h1></div><span class="state-badge">{{ label(detail.state) }}</span></header>
        <p v-if="detailError" class="queue-inline-status queue-inline-status--error" role="alert">{{ detailError }}</p>
        <form v-if="editorOpen" class="schedule-editor decision-card" @submit.prevent="submitDefinition"><h2>Edit schedule</h2><ScheduleFields :model="draft" @update="Object.assign(draft, $event)" /><div class="modal-actions"><IoButton type="button" kind="secondary" @click="editorOpen = false">Cancel</IoButton><IoButton type="submit" :disabled="saving">{{ saving ? "Saving…" : "Save schedule" }}</IoButton></div></form>
        <div v-else class="detail-layout">
          <article class="detail-card schedule-detail"><h2>When it runs</h2><dl><div><dt>Recurrence</dt><dd>{{ time(detail) }}</dd></div><div><dt>Timezone</dt><dd>{{ detail.timezone }}</dd></div><div><dt>Next run</dt><dd>{{ date(detail.next_run_at) }}</dd></div><div><dt>Daylight-saving gap</dt><dd>{{ label(detail.recurrence.gap_policy) }}</dd></div><div><dt>Repeated local time</dt><dd>{{ label(detail.recurrence.overlap_policy) }}</dd></div><div><dt>Missed run</dt><dd>{{ label(detail.missed_run_policy) }}</dd></div><div><dt>Version</dt><dd>{{ detail.version }}</dd></div></dl><details><summary>Agent run template</summary><dl><div><dt>Subject</dt><dd>{{ detail.template.subject }}</dd></div><div><dt>Mode</dt><dd>{{ label(detail.template.mode) }}</dd></div><div><dt>Boardroom</dt><dd class="digest">{{ detail.template.boardroom_id }}</dd></div><div><dt>Personas</dt><dd>{{ detail.template.persona_ids.length }}</dd></div></dl><p>{{ detail.template.prompt }}</p></details></article>
    ...
    <template v-else>
      <header class="page-heading page-heading--action"><div><p class="eyebrow">Reliable recurring work</p><h1>Schedules</h1><p>Run an Agent Boardroom at a clear local time, with daylight-saving and missed-run behavior made explicit.</p></div><IoButton v-if="writable" @click="editorOpen = !editorOpen">{{ editorOpen ? "Close" : "New schedule" }}</IoButton></header>
      ...
      <template v-else>
        <form v-if="editorOpen" class="schedule-editor decision-card" @submit.prevent="submitDefinition"><h2>Create a schedule</h2><ScheduleFields :model="draft" @update="Object.assign(draft, $event)" /><p class="form-note">This unsubmitted draft stays only in this browser tab.</p><p v-if="error" class="form-error" role="alert">{{ error }}</p><IoButton type="submit" :disabled="saving">{{ saving ? "Creating…" : "Create schedule" }}</IoButton></form>
        <section v-if="session.selected?.owner_enrollment_required" class="queue-state queue-state--warning"><h2>Secure this owner Account first</h2><p>Add a passkey and save recovery codes before changing Schedules.</p><a href="/app/security?return_to=%2Fapp%2Fschedules">Continue security setup</a></section>
