...
describe("first-login account setup", () => {
  ...
  it("enrolls a text-message factor without asking for a passkey", async () => {
    ...
    expect(wrapper.text()).toContain("By checking this box");
    ...
    expect(wrapper.text()).toContain("will not be sold or shared with third parties or affiliates for promotional or marketing purposes");
    ...
    const send = wrapper.findAll("button").find((button) => button.text() === "Send my code");
    expect(send?.attributes("disabled")).toBeDefined();
    await wrapper.get('input[type="tel"]').setValue("(202) 555-0199");
    await wrapper.get('input[type="checkbox"]').setValue(true);
    await wrapper.get("form").trigger("submit"); await flushPromises();
    expect(api.beginMFAEnrollment).toHaveBeenCalledWith("sms", "(202) 555-0199", true);
