...
describe("Account lifecycle surface", () => {
  it("binds closure to the selected Account version and explicit reason", async () => { const wrapper = mount(LifecycleView); await flushPromises(); await wrapper.findAll("button").find((button) => button.text() === "Request closure")?.trigger("click"); await wrapper.get("[role=dialog] textarea").setValue("Operations concluded."); await wrapper.get("[role=dialog] input").setValue("CLOSE"); await wrapper.get("[role=dialog]").trigger("submit"); await flushPromises(); expect(api.requestAccountClosure).toHaveBeenCalledWith(account.account_id, 7, "Operations concluded."); expect(wrapper.text()).toContain("No closure history"); });
});
