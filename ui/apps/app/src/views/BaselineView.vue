  ...
  <section class="page baseline-interview-page">
    ...
    <div v-else class="baseline-conversation-layout">
      <section class="baseline-chat-card">
        ...
        <ol ref="chat" class="baseline-chat-log" aria-label="Business Baseline conversation" tabindex="0"><li v-for="item in visibleMessages" :key="item.id" :class="['baseline-message', `baseline-message--${item.role}`]"><div><p class="baseline-message-meta"><strong>{{ messageName(item) }}</strong><time :datetime="item.created_at">{{ messageTime(item.created_at) }}</time></p><p class="baseline-message-body">{{ messageBody(item) }}</p></div></li><li v-if="runPending" class="baseline-message baseline-message--persona"><div><p class="baseline-message-meta"><strong>{{ persona?.name ?? "Operations Guide" }}</strong></p><p class="baseline-thinking"><span></span><span></span><span></span><b class="sr-only">Thinking</b></p></div></li></ol>
