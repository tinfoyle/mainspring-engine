...
<template>
  <section class="page baseline-page">
    ...
    <template v-else>
      <nav class="baseline-progress" aria-label="Baseline progress"><ol><li v-for="(stage, index) in stages" :key="stage" :class="{ done: index < currentStage, current: index === currentStage }"><span>{{ index + 1 }}</span><small>{{ label(stage) }}</small></li></ol></nav>
