...
async function recordCompletedSecurityEnrollment(previouslyReady: boolean): Promise<void> {
  ...
  try {
    ...
    // Optional analytics never delays or changes identity-security setup.
  }
}
...
<template>
  <section class="page security-page">
    ...
    <header class="page-heading"><h1>Security</h1><p>Manage sign-in methods, recovery codes, and connected apps for your login.</p></header>
    ...
    <template v-else>
      ...
      <section class="security-posture"><div><h2>{{ ownerReady ? "Sign-in protection is set up" : "Setup incomplete" }}</h2><p>{{ identity?.primary_email }}</p></div><dl><div><dt>Passkeys</dt><dd>{{ posture?.passkey_count ?? 0 }}</dd></div><div><dt>Code methods</dt><dd>{{ posture?.mfa_method_count ?? 0 }}</dd></div><div><dt>Recovery codes</dt><dd>{{ posture?.recovery_codes_remaining ?? 0 }}</dd></div></dl></section>

      <div class="security-grid">
        <section id="security-confirmation" class="security-card"><h2>Confirm your identity</h2><p v-if="identity?.has_password">Password confirmation unlocks factor recovery. A passkey, text, or email code unlocks privileged Account changes for ten minutes.</p><p v-else>Your identity does not have a Spyglass password. Use a passkey, text message, or email code to confirm sensitive changes.</p><form v-if="identity?.has_password" @submit.prevent="submitPassword"><label>Current password<input v-model="password" type="password" autocomplete="current-password" minlength="12" required></label><IoButton type="submit" :disabled="saving">Confirm password</IoButton></form><IoButton v-if="passkeys.length" kind="secondary" :disabled="saving" @click="confirmPasskey">Confirm with a passkey</IoButton><IoButton v-for="method in mfaMethods" :key="method.id" kind="secondary" :disabled="saving" @click="sendMFACode(method)">Send code to {{ method.destination_hint }}</IoButton><form v-if="mfaChallenge" @submit.prevent="confirmMFACode"><p>Enter the 6-digit code sent to {{ mfaChallenge.destination_hint }}.</p><p v-if="mfaChallenge.development_code" class="form-note">Local test code: {{ mfaChallenge.development_code }}</p><label>Security code<input v-model="mfaCode" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" required></label><IoButton type="submit" :disabled="saving || mfaCode.length !== 6">Confirm code</IoButton></form></section>
