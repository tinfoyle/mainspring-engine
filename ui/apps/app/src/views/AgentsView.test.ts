...
describe("Agents surface", () => {
  ...
  it("publishes a new immutable Persona with bounded proposal policy", async () => {
    ...
    const selects = modal.findAll("select"); await selects[1]!.setValue("propose"); await flushPromises(); const activation = modal.findAll("label").find((item) => item.text().includes("activating a marketing release")); expect(activation).toBeTruthy(); await activation!.find("input").setValue(true); await modal.trigger("submit"); await flushPromises();
