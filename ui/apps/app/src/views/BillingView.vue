...
<template>
  <section class="page billing-page">
    ...
    <header class="page-heading"><h1>Billing</h1><p>Review your plan, usage, and payments. Manage your subscription through Stripe.</p></header>
    ...
    <section v-else-if="purchaseReturned" class="queue-state" role="status"><h2>{{ returnedPurchaseKind }} payment returned</h2><p>Waiting for payment confirmation. Refresh to check the latest status.</p><IoButton kind="secondary" @click="load">Refresh purchase state</IoButton></section>
    ...
    <template v-else-if="billing">
      ...
      <section v-if="billing.lifecycle" class="queue-state" :class="{ 'queue-state--error': billing.lifecycle.state === 'termination_pending' }" role="status">
        <h2>{{ lifecycleTitle }}</h2><p>{{ lifecycleMessage }}</p>
      ...
      <section class="billing-summary-card">
        <div><h2>{{ managed.length ? `${managed.length} managed subscription${managed.length === 1 ? '' : 's'}` : 'Checkout required' }}</h2><p>{{ billing.has_customer ? "This Account has a Stripe customer record." : "No subscription is active. Infinite Ocean has no free plan." }}</p></div>
      ...
      <section class="billing-history" aria-labelledby="ai-token-heading">
        <header><div><h2 id="ai-token-heading">Infinite Ocean AI Tokens</h2></div><span v-if="tokens">{{ quantity(tokens.available) }} available</span></header>
        <p>AI Tokens are credits your team uses for agent tasks. More complex tasks use more credits.</p>
        ...
        <div v-if="catalog" class="billing-token-offers">
          ...
          <small>Tokens are never purchased automatically. Review the price before paying in Stripe.</small>
        ...
        <form v-if="catalog" class="billing-promotion" @submit.prevent="redeemPromotion">
          <div><h3>Redeem a promotion code</h3><p>Enter a promotion code to check whether it is available for your account.</p></div>
      ...
      <section v-if="catalog?.commissioning_offer" class="billing-history" aria-labelledby="commissioning-heading">
        <header><div><h2 id="commissioning-heading">Assisted setup</h2></div><span>One time</span></header>
        <div class="billing-commissioning">
          <div><strong>{{ money(catalog.commissioning_offer.amount_minor, catalog.commissioning_offer.currency) }}</strong><p>{{ catalog.commissioning_offer.disclosure }}</p><small>This service does not change your software plan.</small></div>
          <span v-if="billing.commissioning_purchased" class="state-badge">Purchased</span>
          <IoButton v-else kind="secondary" :disabled="!canPurchase || actionPending" @click="startPurchase('commissioning', catalog.commissioning_offer.code)">{{ purchasing === `commissioning:${catalog.commissioning_offer.code}` ? "Opening Stripe…" : "Buy assisted setup" }}</IoButton>
        </div>
      ...
      <section class="billing-history">
        <header><div><h2>Subscription history</h2></div><span>{{ billing.subscriptions.length }} records</span></header>
