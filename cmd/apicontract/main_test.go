...
func TestEveryCustomerOperationRemainsTyped(t *testing.T) {
	...
	if len(routes) != 219 {
		t.Fatalf("customer operation count = %d, want 219", len(routes))
	}
	for _, route := range routes {
		if route.Contract != "typed" {
			t.Errorf("%s %s regressed to %q contract", route.Method, route.Path, route.Contract)
...
func TestOperationsContractIsSeparateAndTyped(t *testing.T) {
	root, err := repositoryRoot()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, route := range routes {
		...
		if route.Authentication != "operationsCookie" && !strings.Contains(route.OperationID, "PasskeyLogin") && !strings.HasPrefix(route.OperationID, "operationsAdmin") {
