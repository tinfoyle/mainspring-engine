...
"${compose[@]}" run --rm --no-deps --entrypoint /bin/sh object-store-init -ec '
  ...
  mc alias set export-worker http://object-store:9000 "$SPYGLASS_ACCOUNT_EXPORT_OBJECT_STORE_WORKER_ACCESS_KEY" "$SPYGLASS_ACCOUNT_EXPORT_OBJECT_STORE_WORKER_SECRET_KEY" >/dev/null
  ...
  printf export | mc pipe "export-worker/$SPYGLASS_ACCOUNT_EXPORT_OBJECT_STORE_BUCKET/$export_prefix/artifact.zip" >/dev/null
  mc cat "export-worker/$SPYGLASS_ACCOUNT_EXPORT_OBJECT_STORE_BUCKET/$export_prefix/artifact.zip" >/dev/null
  ...
  if mc ls "export-worker/$SPYGLASS_ACCOUNT_EXPORT_OBJECT_STORE_BUCKET" >/dev/null 2>&1; then
    echo "Account export worker object policy permitted bucket listing" >&2
    exit 1
  fi
  if printf denied | mc pipe "export-worker/$SPYGLASS_ACCOUNT_EXPORT_OBJECT_STORE_BUCKET/arbitrary/object" >/dev/null 2>&1; then
    echo "Account export worker object policy permitted an arbitrary-key write" >&2
    exit 1
  fi
  if mc cat "export-worker/$SPYGLASS_OBJECT_STORE_BUCKET/$marketing_prefix" >/dev/null 2>&1; then
    echo "Account export worker object policy permitted a document-bucket read" >&2
    exit 1
  fi
  mc rm --force "export-worker/$SPYGLASS_ACCOUNT_EXPORT_OBJECT_STORE_BUCKET/$export_prefix/artifact.zip" >/dev/null
