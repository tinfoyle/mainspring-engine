...
`make verify-identity-browser` runs a digest-pinned Playwright container inside the local Caddy network namespace. Chromium, Firefox, WebKit, a Chromium phone profile and a test-only Chromium 200% text profile exercise the exact Go-rendered login, signup, verification and recovery surfaces over `https://app.infiniteocean.localhost`; the gate creates no User, submits no password and contacts no deployed environment. Its thirteen checks cover browser-computed WCAG A/AA rules including contrast, responsive overflow, text enlargement, skip-link focus, equal/reversible analytics rejection, offer continuity, native validation and incomplete-link recovery. The text profile bypasses CSP only inside Playwright so it can inject the enlargement rule; the application CSP remains unchanged. The full `make verify` certificate includes this gate after the local stack is rebuilt.
