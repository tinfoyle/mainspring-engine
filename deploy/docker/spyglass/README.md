...
`make verify-identity-browser` runs a digest-pinned Playwright container inside the local Caddy network namespace. Chromium, Firefox, WebKit, a Chromium phone profile and a test-only Chromium 200% text profile exercise the exact Go-rendered login, signup, verification and recovery surfaces over `https://app.infiniteocean.localhost`. Its seventeen checks cover browser-computed WCAG A/AA rules including contrast, responsive overflow, text enlargement, skip-link focus, equal/reversible analytics rejection, offer continuity, native validation and incomplete-link recovery. Each standard browser also creates a uniquely named synthetic User and Account, receives the encrypted queued verification and recovery messages through the TLS Mailpit fixture, consumes both one-time links, enters the real Vue Your Turn route, proves the former password is rejected after recovery and signs in with the replacement. Consumed messages are deleted immediately, and the suite disables traces so credential-bearing URLs cannot enter an artifact. The text profile bypasses CSP only inside Playwright so it can inject the enlargement rule; the application CSP remains unchanged. No deployed environment is contacted. The full `make verify` certificate includes this gate after the local stack is rebuilt.
