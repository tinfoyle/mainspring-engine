...
`make test` uses a disposable PostgreSQL container, the private versioned MinIO service, the constrained Apache Tika extractor, and a pinned Go build image. It runs the uncached Go suite (including PostgreSQL, S3-compatible object-store, and text/PDF/DOCX extraction integration tests), race suite, vet, formatting check, OpenAPI generation/registration check, and the website build/render/accessibility/lint/audit gate without host Go or Node installations.

The digest-pinned Tika service is enabled only by the `test` and `knowledge-processing` profiles until the document-processing worker is wired. It is private, read-only, capability-free, capped at 1.5 GiB/2 CPUs/256 processes, uses a bounded tmpfs, disables unsecure Tika features and stack-trace responses, and restarts its forked parser after a two-minute task timeout or 1,000 files.
