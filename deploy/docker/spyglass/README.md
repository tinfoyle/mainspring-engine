...
`make test` uses a disposable PostgreSQL container, the private versioned MinIO service, the private ClamAV scanner, the constrained Apache Tika extractor, and a pinned Go build image. It certifies that the admission credential can write source objects but not extracted objects while the worker credential can read source and write extracted objects. It then runs the uncached Go suite (including PostgreSQL, S3-compatible object-store, clean/infected malware-stream, and text/PDF/DOCX extraction integration tests), race suite, vet, formatting check, OpenAPI generation/registration check, and the website build/render/accessibility/lint/audit gate without host Go or Node installations.

The `knowledge-processing` profile runs one least-privilege document worker per cell with the digest-pinned Tika and ClamAV services; stage deployment enables that profile. Each worker claims only content-free queue records and is the sole process combining exact-version object access, scanning, extraction and deterministic indexing. Tika is private, read-only, capability-free, capped at 1.5 GiB/2 CPUs/256 processes, uses a bounded tmpfs, disables unsecure features and stack-trace responses, and restarts its forked parser after a two-minute task timeout or 1,000 files. ClamAV is private, capped at 4 GiB/2 CPUs/256 processes, limits a source stream to 50 MiB and a scan to 100 MiB/two minutes, and persists signature data. Deterministic local/CI tests use the signatures pinned in the image; stage enables FreshClam updates on the persistent volume.
...
`env/local.env` contains intentionally public, local-only credentials and deterministic keys. It must never be copied to stage or production. Environment-specific secret files are supplied separately. `prepare-stage-secrets.sh` generates separate MinIO administrator, source-admission and document-worker credentials plus the static server-side-encryption key. Initialization attaches prefix-scoped policies to the two workload users; the administrator credential remains confined to MinIO and its one-shot initializer. MinIO is never published at a host port, its bucket is initialized with versioning enabled, and its persistent volume survives ordinary stack shutdown. Production uses corresponding scoped Linode Object Storage credentials rather than this Compose service.
