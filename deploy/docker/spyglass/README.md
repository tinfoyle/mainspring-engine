...
`make verify-identity-browser` runs a digest-pinned Playwright container inside the local Caddy network namespace. Chromium, Firefox, WebKit and a Chromium phone profile exercise the exact Go-rendered login, signup, verification and recovery surfaces over `https://app.infiniteocean.localhost`; the gate creates no User, submits no password and contacts no deployed environment. It checks browser-computed WCAG A/AA rules including contrast, responsive overflow, skip-link focus, equal/reversible analytics rejection, offer continuity, native validation and incomplete-link recovery. The full `make verify` certificate includes this gate after the local stack is rebuilt.
