...
for _ in $(seq 1 30); do
  ...
  if jq -e '[.data.activeTargets[] | select(.health == "up")] | length == 17' <<<"$targets" >/dev/null; then
    break
  fi
  sleep 1
done
...
jq -e '
  (.status == "success") and
  (.data.activeTargets | length == 17) and
...
jq -e '.status == "success" and .data.resultType == "vector" and .data.result[0].value[1] == "17"' <<<"$query" >/dev/null
