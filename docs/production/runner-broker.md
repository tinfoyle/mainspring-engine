...
Commercial extension decision (2026-08-26): the trusted normalized provider usage and internal cost remain inputs to a separate versioned Infinite Ocean AI Token rate card. Customer balances never expose or depend directly on a vendor token denomination. Admission reserves the bounded customer-token maximum under one frozen rate version; exact terminal settlement debits customer AI Tokens and records provider cost separately. Kimi, Z.ai and future adapters must satisfy the same normalized usage, strict structured-output/tool, no-storage, idempotency and failure contracts before they can enter a reviewed rate card.

Launch model-presentation correction (2026-08-26): provider and model identifiers remain internal and cannot be selected by a browser/API client. Each Agent freezes one validated complexity code from `simple`, `efficient`, `balanced`, `thorough`, or `advanced`; new Agents default to `balanced`. The effective reviewed rate card maps that class to a certified provider/model/adapter and AI Token schedule. A reviewed substitution still requires exact-adapter certification for output schemas, sequential tools, privacy/no-storage, usage/cost normalization, cancellation and replay. Each admitted invocation freezes complexity plus provider/model/adapter/rate versions. Retries and reconciliation use that execution identity; later customization or a new mapping affects future admissions only.
