...
Only `work.summary.read` is registered. Consequential handlers remain impossible to construct without an action authorizer, and no consequential definition is mounted. The next slice must add the durable approval/action ledger before any effectful tool, add the `runner-invocation` process with real kind executors, define terminal exchange/audit/tool-receipt retention, and enforce capability policy at provisioning rather than accepting caller-selected capabilities.
