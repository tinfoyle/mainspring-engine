...
Launch model-presentation decision (2026-08-26): the customer surface offers only `standard-ai`; provider and model identifiers remain internal operational configuration and cannot be selected by a browser/API client. A reviewed substitution requires exact-adapter certification for output schemas, sequential tools, privacy/no-storage, usage/cost normalization, cancellation and replay. Each admitted invocation freezes the selected provider/model/adapter and AI Token rate versions. Retries and reconciliation use that frozen execution identity; a new default affects future admissions only.
