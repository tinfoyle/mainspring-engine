...
Status: route, owner, authentication, reference, and generation drift gates executable; every cell-owned operation, the public Account-entry journey, and system-identity security operations are typed; remaining global Account API payload certification continues
...
Every operation declares a unique operation ID, owning runtime, authentication class, and success/problem responses. Path identifiers are typed and required. Account context and every Work and Agents read/mutation declare exact query/header/body/status/response contracts. The public Catalog, registration start/completion, password/passkey login, Account choices/selection, passkey enrollment/list/removal/reauthentication, enumeration-resistant credential recovery, recovery-code posture/use, factor posture, session inventory/revocation, security history, password reauthentication, and logout operations do as well. This includes the free-Account provisioning result, owner-enrollment gate, bounded WebAuthn wire envelopes, explicit Origin requirements for cookie-authenticated mutations, and the intentionally unauthenticated idempotent logout boundary. Persona publication exposes only caller-owned model/tool/citation/action policy; Spyglass injects and owns the immutable result schema. The remaining global Account operations still use a shared success response that intentionally promises no body or exact status while their payloads are certified operation by operation; the whole contract must not yet be presented as a fully typed third-party SDK promise.
