...
Status: all 186 customer-facing operations have typed success contracts; route, owner, authentication, reference, generation, and no-generic-response drift gates are executable
...
Every operation declares a unique operation ID, owning runtime, authentication class, exact success status and payload, and stable Problem envelope. Path identifiers are typed and required. The contract covers public Catalog and Account acquisition; password, passkey, recovery, verified-contact change, and session security; Account selection, Memberships, invitations, ownership, closure, and Billing; Stripe webhook acceptance; and cell-owned Account context, Work, and Agents. This includes the free-Account provisioning result, owner-enrollment gate, bounded WebAuthn wire envelopes, passkey-protected new-mailbox verification and session-revocation result, explicit Origin requirements for cookie-authenticated mutations, idempotency and optimistic-concurrency headers, the intentionally unauthenticated idempotent logout boundary, stable Conversation/Message cursors, immutable Run recovery decisions, and safe external DTOs instead of internal aggregates. Persona publication exposes only caller-owned model/tool/citation/action policy; Spyglass injects and owns the immutable result schema, and the Message contract preserves its typed evidence/action/delegation envelope. Run reads expose customer-safe invocation lifecycle without making raw provider failure codes part of the public contract. The Integration authorization slice includes manager/recent-passkey begin and revoke operations, a secret-free status read and one exact HTTP-only Google callback. State, PKCE verifier, authorization code, provider tokens and broker references are absent from public schemas.
