...
Status: route, owner, authentication, reference, and generation drift gates executable; every cell-owned Account-context, Work, and Agents operation is typed; global Account API payload certification remains
...
Every operation declares a unique operation ID, owning runtime, authentication class, and success/problem responses. Path identifiers are typed and required. Account context and every Work and Agents read/mutation now declare exact query/header/body/status/response contracts. Persona publication exposes only caller-owned model/tool/citation/action policy; Spyglass injects and owns the immutable result schema. The remaining global Account operations still use a shared success response that intentionally promises no body or exact status while their payloads are certified operation by operation; the whole contract must not yet be presented as a fully typed third-party SDK promise.
...
The check command parses the Go Account/cell transport registrations with the Go AST and compares every customer JSON method/path/service tuple with OpenAPI. It also rejects duplicate operation IDs, missing ownership, missing security declarations, missing responses, unresolved or external schema references, stale generated output, and undocumented or unregistered routes. A regression test keeps the Account-context, seven Work operations, and six Agents operations on their typed contracts. CI runs the check before accepting a change.
