...
Status on 2026-08-23: **in progress.** The durable authorization-session aggregate, encrypted provider-secret port, fixed-origin Google protocol adapter and manager/recent-passkey begin boundary are complete. The callback now claims the state once, freezes a credential-generation plan, seals the returned refresh material and atomically activates or rotates the durable credential/connection/session records. Migration 73 adds a forced-RLS, non-portable recovery ledger and blocks competing connection authority changes while a callback is unsettled. Provider exchange is durably marked before network I/O and never repeated after an uncertain result; an exact vault-presence check recovers an ambiguous secret write, while a stale exchange without sealed material becomes one terminal content-free failure. Rotation seals the next generation, fences the previous vault generation, records that fence, then converges the PostgreSQL transaction on retry. Focused race and fresh PostgreSQL tests cover exact activation/replay, generation-two rotation, required prior fencing, changed code/state, concurrent exchange leases, scope failure, ambiguous writes, unknown outcomes, connection mutation fencing, RLS, portability omission and immutable redacted events. Status/revocation operations, HTTP/MCP transports and the deterministic containerized OAuth-to-Drive fixture remain.
