...
Status on 2026-08-23: **in progress.** The durable authorization-session aggregate and migration are complete. The provider-secret port now also has a local production-shaped AES-256-GCM filesystem implementation: its key, root and sealed files require restrictive non-symlink paths; authorization state/PKCE verifiers and credential generations are separated; writes are atomic and replay-safe; and rotation/revocation fencing stops further leases before later purge. Lease requests continue to require the exact Account, connection, credential generation, provider, reference digest, purpose, capability and expiry. A fixed-origin Google protocol adapter constructs exact offline S256 consent, exchanges one-use codes through the reviewed token endpoint, requires the single Drive-read scope and revokes refresh material without following redirects. The application and Account-RLS repository now complete the manager/recent-passkey begin boundary: current Drive authority is frozen, only digests reach PostgreSQL and exact retries recover the same encrypted state/verifier and reproduce the same consent URL after an ambiguous write or process interruption. Focused race and fresh-database tests prove ciphertext tamper denial, no plaintext at rest, exact replay/conflict behavior, expiry, rotation/revocation fencing, purge replay, Drive health/sync purpose separation, real Google-style authorization-code syntax, redirect refusal, bounded responses, scope drift, revocation replay, durable begin replay and redacted event persistence. Callback claim/exchange completion, credential binding/compensation, status/revoke transports and the deterministic containerized provider fixture remain.
