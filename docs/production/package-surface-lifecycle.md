...
The current repository has no customer-facing package export endpoint. Production MCP, connector execution and the PostgreSQL-backed Knowledge search surface are now implemented elsewhere in Phase 3 and are no longer part of this historical closed list. “Not present” remains a closed state: no route, tool or network grant exists to bypass the matrix. Phase 3 must register each new surface here before implementation and must add enabled/read-only/suspended tests at its outermost transport and durable admission boundary. Customer-created Agent schedules are executable and therefore intentionally absent from this closed list.
