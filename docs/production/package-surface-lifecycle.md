...
Production MCP and the Account-level customer export API are executable and no longer appear in the machine-readable absent list. Connector execution remains local-certification-only; no real provider adapter is enabled. External vector search and analytics export stores remain absent. “Not present” remains a closed state: no route, tool or network grant exists to bypass the matrix. Every new surface must be registered here before implementation and must add enabled/read-only/suspended tests at its outermost transport and durable admission boundary. Customer-created Agent schedules are executable and therefore intentionally absent from this closed list.
