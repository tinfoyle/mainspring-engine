...
Production MCP, the Account-level customer export API and the dedicated connector runtime are constructed and no longer appear in the machine-readable absent list. The connector runtime is still activation-closed: Marketing and Integrations remain non-executable until the real SMTP/HTTPS path has applied Stage provider, recovery and erasure evidence. External vector search and analytics export stores remain absent. “Not present” remains a closed state: no route, tool or network grant exists to bypass the matrix. Every new surface must be registered here before implementation and must add enabled/read-only/suspended tests at its outermost transport and durable admission boundary. Customer-created Agent schedules are executable and therefore intentionally absent from this closed list.

Work, Agents, Knowledge and Finance have executable browser, HTTP and MCP package surfaces. Marketing and Integrations remain non-executable until the closed connector runtime receives reviewed real provider adapters and applied certification. Public Catalog publication exposes stable descriptions, package dependencies, capabilities, offer presentation and governed limit definitions; it never exposes Stripe Product/Price IDs, provider credentials, internal support overrides, Account grants or unpublished drafts.
