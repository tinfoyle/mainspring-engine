...
| MCP gateway and cell tools | Work, Agents, Knowledge, Finance | Typed package reads remain available under the same `read_only` rules as HTTP | Every tool is classified before routing and every cell application service repeats package/mutation authorization | OAuth bearer authority is Account-bound; the gateway signs a one-use route proof and never forwards the bearer token |
...
| Browser, HTTP and MCP | Marketing | Retained campaigns, immutable creative metadata and release evidence remain readable | Drafting, submission, approval and campaign transitions are denied | Every transport calls the same package-authorized application service; immutable asset admission remains exact-version and content-addressed |
| Agent tool and approved-action worker | Marketing | Bounded campaign, asset and release reads remain available to an already-admitted Run | A read-only package cannot admit a new draft or approval action | Runner authority is invocation/Run-bound; campaign activation consumes only an exact approved Attention action |
| Browser, HTTP and MCP | Integrations | Non-secret connection scope, health and execution evidence remain readable | Connection, credential-binding, execution preparation and recovery mutations are denied | Both gateways classify the Integrations package before the cell repeats current Account/package authorization |
| Integration connector worker | Marketing plus Integrations | May inspect only the frozen execution, immutable release assets and content-free connector state needed to settle admitted work | Cannot prepare a new execution or widen a release, connection revision, credential generation or capability | Current package/placement authority, fresh health, exact object version and one-operation credential leases are rechecked before execute/reconcile; unknown effects are never blindly resent |
| Integration source and research paths | Integrations plus Knowledge where capture mutates Knowledge | Scoped source/research status and retained provenance remain available | New source grants, searches and Knowledge capture are denied when the owning package is read-only/absent | Drive, IMAP and public-web adapters consume exact immutable scope; Knowledge admission repeats its own package mutation authority |
...
Production MCP, the Account-level customer export API and the dedicated connector runtime are constructed and no longer appear in the machine-readable absent list. Marketing and Integrations are executable in the local backend inventory after their HTTP/MCP/browser, immutable-object, worker, provider-adapter, health, recovery, export and erasure gates passed. This is a construction claim, not Stage or production release evidence. External vector search and analytics export stores remain absent. “Not present” remains a closed state: no route, tool or network grant exists to bypass the matrix. Every new surface must be registered here before implementation and must add enabled/read-only/suspended tests at its outermost transport and durable admission boundary. Customer-created Agent schedules are executable and therefore intentionally absent from this closed list.

Work, Agents, Knowledge, Finance, Marketing and Integrations now have executable declared surfaces. Marketing and Integrations additionally declare their bounded Agent/action and connector-worker paths. Environment release remains separately gated by immutable artifacts, applied provider acceptance and the Stage/LKE certificates. Public Catalog publication exposes stable descriptions, package dependencies, capabilities, offer presentation and governed limit definitions; it never exposes Stripe Product/Price IDs, provider credentials, internal support overrides, Account grants or unpublished drafts.

## Export, retention and restoration rule

Customer export is an Account-level lifecycle capability, not a paid-package permission. [ADR-0009](decisions/0009-account-portability-export.md), the executable builder, durable request/expiry state, private exact-version artifact adapter, private staging/pipeline kernel, global/cell repeatable-read coordinator, complete global/cell database projections and exact Knowledge/Marketing source factory fix the deterministic archive, bounded snapshot, source-integrity, lease recovery, movement fencing, fail-closed schema/column registries and create-if-absent publication/deletion contracts. Per-cell build and global expiry workers are composed locally, for Hostinger Stage and in the review-only LKE reference with distinct least-privilege database/object identities and private ephemeral staging. Owner-only HTTP, MCP and private-browser surfaces share one request/history/status/cancel/capability boundary; mutations and downloads require recent passkey evidence, and the header-only capability is state-bound to the exact immutable artifact. `customer-export-api` is therefore executable even when every commercial package is absent or suspended.
