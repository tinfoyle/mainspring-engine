...
| Team Account and customer data | Provide the paid Spyglass workspace and perform guarded lifecycle erasure | Contract/service necessity | Account identity, Memberships, package data, customer content, lifecycle/export state and cross-store erasure evidence. Infinite Ocean-operated stores and approved subprocessors only. | First unresolved renewal failure starts a 30-day clock with seven-day read-only remediation. Voluntary cancellation starts its 30-day clock only at paid period end. Recovery/resubscription before day 30 cancels deletion; otherwise guarded Account erasure deletes live customer data. Then delete the live User identity only if it has no other Account Membership and no active Affiliate relationship. Required financial/Affiliate/security/legal-hold evidence follows separate restricted retention. |
