...
Customer support inspection requires a passkey-authenticated staff User with `support` or `operations_administrator`, an exact User/Account lookup, a support ticket and plain-language reason, an exact User and Account scope, and a five-to-sixty-minute grant. The UI defaults to fifteen minutes.
...
Role governance is deliberately offline. The target must already be an active ordinary User with a passkey. Use the migration credential only for this command; never place it in the console service.
...
- provision passkey-capable staff identities and grant roles offline;
