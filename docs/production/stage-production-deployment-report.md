...
The Operations Console is locally feature-complete but is not part of Stage RC.34. Do not add it to Hostinger Stage or LKE until the product owner authorizes that separate surface. A future deployment requires a dedicated Operations subdomain/origin, separate passkey-only cookie boundary, five unique least-authority PostgreSQL credentials, offline staff-role provisioning, environment-specific monitoring/backups and completion of the gates in [Operations Console operations](operations-console-operations.md). Its customer, public and staff origins must remain distinct.
...
RC.34 deploys the Baseline reply-reconciliation repair from source revision `d899e2994d688af02e061593f211cff1af794b66` and tracked manifest/checkout revision `e586e93e09087c5f405f1eb348280c527effa5b3`. The application, public UI and private UI are pinned by digest in `deploy/releases/0.3.0-rc.34.env`, carry BuildKit SBOM/provenance attestations, and passed the publisher's high/critical vulnerability and secret scans with zero findings. Historical approved setup tasks are resolved by stable Agent-message ID and no longer replay creation after later link/assignment changes; concurrent repair loads the existing Work item, and a durable reply replaces its optimistic copy before Work synchronization. The protected configuration verified before and after deployment. All 48 healthchecked workloads are healthy among 49 running containers; the three retained PostgreSQL ledgers remain at global migration 68 and cell migrations 80/80. Public, login and MCP protected-resource metadata return `200`, and all three live image identities equal the manifest digests. Authenticated browser evidence confirms one durable copy of the affected reply, two linked Work items, no Work conflict and an empty console.
...
|---|---|---|
| Application | `0.3.0-rc.34` | `ghcr.io/tinfoyle/spyglass-engine@sha256:83308a98dd516623c8ffddd72f08c8f7ecc94a965c890610141196aca263ac0b` |
| Public UI | `0.3.0-rc.34` | `ghcr.io/tinfoyle/infinite-ocean-public-ui@sha256:98a7ac27d65372d9eb616e81911e57eb04b95c35a009555ac3de3f063d7b5749` |
| Private UI | `0.3.0-rc.34` | `ghcr.io/tinfoyle/infinite-ocean-private-ui@sha256:071563a9d575cbe2301ebdb39ff65a3c199869e43767f895aab503457253a4f1` |

The RC.34 triple was built from source revision
`d899e2994d688af02e061593f211cff1af794b66` and is tracked in
`deploy/releases/0.3.0-rc.34.env`. It passed local AMD64 admission and connected
...
The historical connected-stage RC.5 -> RC.4 -> RC.5 rehearsal completed successfully against retained PostgreSQL volumes. RC.4 and the restored RC.5 each passed the same 11-check exact-origin/Catalog boundary certificate. RC.5 remains the Phase 2.5 platform rollback baseline; RC.34 is active on Stage, and its expanded schema does not inherit that earlier downgrade proof. The final production release will be a later signed immutable triple; no current candidate is predeclared as that release.
...
The same matched application/public/private digests must move through local validation, Hostinger Stage and LKE production configuration without rebuilding. The final operator release path must supply attached SBOM, BuildKit provenance, a trusted signature and retention evidence; the unsigned single-platform RC.34 Stage artifact does not satisfy that final gate.
