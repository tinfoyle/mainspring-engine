...
The Operations Console is locally feature-complete but is not part of Stage RC.33. Do not add it to Hostinger Stage or LKE until the product owner authorizes that separate surface. A future deployment requires a dedicated Operations subdomain/origin, separate passkey-only cookie boundary, five unique least-authority PostgreSQL credentials, offline staff-role provisioning, environment-specific monitoring/backups and completion of the gates in [Operations Console operations](operations-console-operations.md). Its customer, public and staff origins must remain distinct.
...
RC.33 deploys the adaptive `Operations Guide` interaction repair from source revision `b84be1dc5b1a79d2947c2bca2d57d64d4deae2a7` and tracked manifest/checkout revision `100c5dc61de9ed6822b73898b679235ba7847232`. The application, public UI and private UI are pinned by digest in `deploy/releases/0.3.0-rc.33.env`, carry BuildKit SBOM/provenance attestations, and passed the publisher's high/critical vulnerability and secret scans with zero findings. Human replies now render immediately while the Agent is working; automation acceptance is a compact chat event and accepted offers cannot be fired repeatedly during a pending run. The application router now admits the existing Work conversation/provenance link commands, and reload-based idempotent reconciliation repaired the two previously stranded setup tasks without duplicates. The protected configuration verified before and after deployment. All 48 healthchecked workloads are healthy among 49 running containers; the three retained PostgreSQL ledgers remain at global migration 68 and cell migrations 80/80. Public, login and MCP protected-resource metadata return `200`, and the live application/private UI container identities equal the manifest digests. Authenticated browser evidence confirms compact approval events, two linked Work items, no route error and an empty console.

This is an authenticated owner-review candidate. It now certifies connected provider-backed Knowledge capture and approved Work creation for the reviewed slice, but does not yet certify completion of the adaptive interview or Your Turn continuation, and it does not authorize LKE or production deployment.
...
|---|---|---|
| Application | `0.3.0-rc.33` | `ghcr.io/tinfoyle/spyglass-engine@sha256:162a772a3f4eb43c5399f787bd46844a3f588dfc694cd2d38c1ceb88937703fa` |
| Public UI | `0.3.0-rc.33` | `ghcr.io/tinfoyle/infinite-ocean-public-ui@sha256:9f95f8060665a143c848f3d874d302da91316aaa5f98875d05c19723cc7d4aa8` |
| Private UI | `0.3.0-rc.33` | `ghcr.io/tinfoyle/infinite-ocean-private-ui@sha256:77cee73148035c2d3584ae8f1b70ada0a1585c76c4105f3ca152a40a8afad979` |

The RC.33 triple was built from source revision
`b84be1dc5b1a79d2947c2bca2d57d64d4deae2a7` and is tracked in
`deploy/releases/0.3.0-rc.33.env`. It passed local AMD64 admission and connected
Stage deployment verification with attached SBOM/provenance. It remains unsigned,
lacks multi-architecture evidence, and still requires authenticated owner acceptance.
...
The historical connected-stage RC.5 -> RC.4 -> RC.5 rehearsal completed successfully against retained PostgreSQL volumes. RC.4 and the restored RC.5 each passed the same 11-check exact-origin/Catalog boundary certificate. RC.5 remains the Phase 2.5 platform rollback baseline; RC.33 is active on Stage, and its expanded schema does not inherit that earlier downgrade proof. The final production release will be a later signed immutable triple; no current candidate is predeclared as that release.
...
The same matched application/public/private digests must move through local validation, Hostinger Stage and LKE production configuration without rebuilding. The final operator release path must supply attached SBOM, BuildKit provenance, a trusted signature and retention evidence; the unsigned single-platform RC.33 Stage artifact does not satisfy that final gate.
