...
The Operations Console is locally feature-complete but is not part of Stage RC.32. Do not add it to Hostinger Stage or LKE until the product owner authorizes that separate surface. A future deployment requires a dedicated Operations subdomain/origin, separate passkey-only cookie boundary, five unique least-authority PostgreSQL credentials, offline staff-role provisioning, environment-specific monitoring/backups and completion of the gates in [Operations Console operations](operations-console-operations.md). Its customer, public and staff origins must remain distinct.
...
## Current connected Stage checkpoint (2026-09-03)

RC.32 deploys the adaptive `Operations Guide` prompt-placement repair from source revision `22df0ef272bb68fcd17b555a4914ec4e84671a61` and tracked manifest/checkout revision `f4d05636b07034558922db88ea58bcbce64d525e`. The application, public UI and private UI are pinned by digest in `deploy/releases/0.3.0-rc.32.env`, carry BuildKit SBOM/provenance attestations, and passed the publisher's high/critical vulnerability and secret scans with zero findings. RC.32 retains the RC.31 null-collection repair, places the structured next question in the Agent chat when needed, avoids duplicating a question already present in Agent prose, and leaves the human reply field empty with neutral guidance. The protected configuration verified before and after deployment. All 48 healthchecked workloads are healthy among 49 running containers; the three retained PostgreSQL ledgers remain at global migration 68 and cell migrations 80/80. Public, login and MCP protected-resource metadata return `200`, and the live application/private UI container identities equal the manifest digests. Authenticated browser evidence confirms the pending question in the Agent bubble, the empty human-only reply field and an empty console.

This is an authenticated owner-review candidate. It does not certify the connected provider-backed adaptive conversation, exact Knowledge capture, approved Work creation, or Your Turn continuation, and it does not authorize LKE or production deployment.
...
|---|---|---|
| Application | `0.3.0-rc.32` | `ghcr.io/tinfoyle/spyglass-engine@sha256:c71984a2ea693d957b6a8700a2be8a1a577f8d5f56e77e82c7a05e382a50ac64` |
| Public UI | `0.3.0-rc.32` | `ghcr.io/tinfoyle/infinite-ocean-public-ui@sha256:51eb68ed80c631b5036cc34f70971f464e2c1df00c46c6e7556c8bc98dfe1480` |
| Private UI | `0.3.0-rc.32` | `ghcr.io/tinfoyle/infinite-ocean-private-ui@sha256:a56fdeb3c60f6b3c16e7cba3f15101944852069f5adbab6e885705de835234c4` |

The RC.32 triple was built from source revision
`22df0ef272bb68fcd17b555a4914ec4e84671a61` and is tracked in
`deploy/releases/0.3.0-rc.32.env`. It passed local AMD64 admission and connected
...
The historical connected-stage RC.5 -> RC.4 -> RC.5 rehearsal completed successfully against retained PostgreSQL volumes. RC.4 and the restored RC.5 each passed the same 11-check exact-origin/Catalog boundary certificate. RC.5 remains the Phase 2.5 platform rollback baseline; RC.32 is active on Stage, and its expanded schema does not inherit that earlier downgrade proof. The final production release will be a later signed immutable triple; no current candidate is predeclared as that release.
...
The same matched application/public/private digests must move through local validation, Hostinger Stage and LKE production configuration without rebuilding. The final operator release path must supply attached SBOM, BuildKit provenance, a trusted signature and retention evidence; the unsigned single-platform RC.32 Stage artifact does not satisfy that final gate.
...
On 2026-08-21 the website and application stage names resolved to the VPS public address and the shared Caddy routes were activated with the repository's shared security-header policy, including HSTS. On 2026-08-22 the MCP wildcard hostname was activated. On 2026-08-27 `/opt/spyglass-stage/current` advanced to the RC.9 checkout and connected browser acceptance confirmed the Your Turn-led Features story, all twelve detail links, canonical metadata and no horizontal overflow. On 2026-09-03 the selector advanced through RC.30 for the adaptive interview and RC.31 for null-collection compatibility, then to `/opt/spyglass-stage/releases/f4d05636b07034558922db88ea58bcbce64d525e` for RC.32 prompt placement. The exact RC.32 application/public/private digest triple is deployed, 49 long-running containers are present, all 48 healthchecked workloads are healthy, the internal edge is running, and the retained global/cell A/cell B PostgreSQL services remain at migrations 68/80/80. Authenticated browser review confirms the pending question inside the Agent chat and an empty human-only reply field; owner acceptance of the complete adaptive Baseline journey remains open.
