...
RC.44 completes the [Stage functional audit and plain-language UI review](stage-functional-audit-2026-09-04.md), from source revision `171992fe96389367e0bd07f0f8edbc2593a7f9e0` and tracked manifest/checkout revision `ab964530a32240239e2b773d30617ccf00294d3d`. The application, public UI and private UI are pinned by digest in `deploy/releases/0.3.0-rc.44.env`, carry BuildKit SBOM/provenance attestations, and passed the publisher's high/critical vulnerability and secret scans with zero findings. Repairs cover saved schedule runs, no-tool agents and summary selection, Marketing content download and human approval, Finance supporting notes, Knowledge fact navigation, and clearer customer-facing copy. Protected configuration verified before and during deployment. All 48 healthchecked workloads are healthy among 49 running containers; PostgreSQL ledgers are at global migration 68 and cell migrations 86/86. Public and login origins return `200`, and all three live image identities equal the manifest digests.

Authenticated live acceptance passed human Work, agent Work with a Your Turn answer and continuation, scheduled execution and history, two-agent summaries, Finance posting/reversal/reconciliation/closure/archive, and Marketing upload/download/approval and campaign lifecycle. The synthetic campaign and ledger are archived, the schedule is paused, and the unbound test integration is revoked. Real payments, external-provider activation/delivery, identity-code or passkey changes, invitations, destructive account actions, and external MCP-client certification remain outside this audit's completed evidence. This is a Stage owner-review candidate; it does not authorize LKE or production deployment.
...
|---|---|---|
| Application | `0.3.0-rc.44` | `ghcr.io/tinfoyle/spyglass-engine@sha256:d31e4599c234f57cea25182864952a5fd37e726e46575589314dca63557e59f4` |
| Public UI | `0.3.0-rc.44` | `ghcr.io/tinfoyle/infinite-ocean-public-ui@sha256:38ea6ec4748eaf75ad001ff2bd06b964c9bd48efae0afc5c72ad251d3d204204` |
| Private UI | `0.3.0-rc.44` | `ghcr.io/tinfoyle/infinite-ocean-private-ui@sha256:365451f54417d7ab5b7dec5d6286f35b63edd9d77e05331d03e304b5a4ed4693` |

The RC.44 triple was built from source revision
`171992fe96389367e0bd07f0f8edbc2593a7f9e0` and is tracked in
`deploy/releases/0.3.0-rc.44.env`. It passed local AMD64 admission and connected
Stage deployment verification with attached SBOM/provenance. It remains unsigned,
lacks multi-architecture evidence, and still requires the remaining provider, security and final-product acceptance evidence.
...
The historical connected-stage RC.5 -> RC.4 -> RC.5 rehearsal completed successfully against retained PostgreSQL volumes. RC.4 and the restored RC.5 each passed the same 11-check exact-origin/Catalog boundary certificate. RC.5 remains the Phase 2.5 platform rollback baseline; RC.44 is active on Stage, and its expanded schema does not inherit that earlier downgrade proof. The final production release will be a later signed immutable triple; no current candidate is predeclared as that release.
