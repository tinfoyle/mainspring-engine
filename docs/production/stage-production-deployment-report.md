...
The Operations Console is locally feature-complete but is not part of Stage RC.41. Do not add it to Hostinger Stage or LKE until the product owner authorizes that separate surface. A future deployment requires a dedicated Operations subdomain/origin, separate passkey-only cookie boundary, five unique least-authority PostgreSQL credentials, offline staff-role provisioning, environment-specific monitoring/backups and completion of the gates in [Operations Console operations](operations-console-operations.md). Its customer, public and staff origins must remain distinct.
...
RC.41 completes agent-owned Work execution from source revision `9e8cd343af2696c35006c1a363338014b66aa3a0` and tracked manifest/checkout revision `8619a6f3735e3f706bc7ecebe18dc5d38063bd6e`. The application, public UI and private UI are pinned by digest in `deploy/releases/0.3.0-rc.41.env`, carry BuildKit SBOM/provenance attestations, and passed the publisher's high/critical vulnerability and secret scans with zero findings. Eligible persona-owned Work now dispatches automatically through the provider-neutral runner path. Successful executions either complete the Work or open a concrete Your Turn question; capacity deferrals no longer consume retries; genuine model failures retry twice; and a third failure opens an owner-facing recovery question. The protected configuration verified before and during deployment. All 48 healthchecked workloads are healthy among 49 running containers; the retained PostgreSQL ledgers are at global migration 68 and cell migrations 85/85. The public and login origins return `200`, and all three live image identities equal the manifest digests. Live database acceptance confirms that the pricing review is waiting on a specific token/cost question and that the previously exhausted launch-readiness review now has its own open recovery request.
...
|---|---|---|
| Application | `0.3.0-rc.41` | `ghcr.io/tinfoyle/spyglass-engine@sha256:18e06aa6954386ead58fa9caabf6e8ae1527308590953c8bd001270098769eca` |
| Public UI | `0.3.0-rc.41` | `ghcr.io/tinfoyle/infinite-ocean-public-ui@sha256:b9382a29b26c6d68cd1b1e1175cdbc73af304609e039802da2dac318176c1f07` |
| Private UI | `0.3.0-rc.41` | `ghcr.io/tinfoyle/infinite-ocean-private-ui@sha256:bd8bc88e813791edf02b52d882dd4ee8d8459742d064dc210020dd55142bc32e` |

The RC.41 triple was built from source revision
`9e8cd343af2696c35006c1a363338014b66aa3a0` and is tracked in
`deploy/releases/0.3.0-rc.41.env`. It passed local AMD64 admission and connected
...
The historical connected-stage RC.5 -> RC.4 -> RC.5 rehearsal completed successfully against retained PostgreSQL volumes. RC.4 and the restored RC.5 each passed the same 11-check exact-origin/Catalog boundary certificate. RC.5 remains the Phase 2.5 platform rollback baseline; RC.41 is active on Stage, and its expanded schema does not inherit that earlier downgrade proof. The final production release will be a later signed immutable triple; no current candidate is predeclared as that release.
...
The same matched application/public/private digests must move through local validation, Hostinger Stage and LKE production configuration without rebuilding. The final operator release path must supply attached SBOM, BuildKit provenance, a trusted signature and retention evidence; the unsigned single-platform RC.41 Stage artifact does not satisfy that final gate.
