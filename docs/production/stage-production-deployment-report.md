...
The Operations Console is locally feature-complete but is not part of Stage RC.35. Do not add it to Hostinger Stage or LKE until the product owner authorizes that separate surface. A future deployment requires a dedicated Operations subdomain/origin, separate passkey-only cookie boundary, five unique least-authority PostgreSQL credentials, offline staff-role provisioning, environment-specific monitoring/backups and completion of the gates in [Operations Console operations](operations-console-operations.md). Its customer, public and staff origins must remain distinct.
...
RC.35 deploys conventional Baseline chat keyboard behavior from source revision `2af23ef1ed18dcf5fff8bd8be84ec2fcee89c89c` and tracked manifest/checkout revision `bc45b13693cf59f303e738154629f55427ce14b7`. The application, public UI and private UI are pinned by digest in `deploy/releases/0.3.0-rc.35.env`, carry BuildKit SBOM/provenance attestations, and passed the publisher's high/critical vulnerability and secret scans with zero findings. Enter submits, Shift+Enter inserts a new line, an active Agent run cannot be double-submitted, and IME composition cannot accidentally send; the composer gives this guidance directly. The RC.34 reply and approved-Work reconciliation repairs remain intact. The protected configuration verified before and after deployment. All 48 healthchecked workloads are healthy among 49 running containers; the three retained PostgreSQL ledgers remain at global migration 68 and cell migrations 80/80. Public, login and MCP protected-resource metadata return `200`, all three live image identities equal the manifest digests, and the first five minutes contain no severe startup log line. A clean authenticated browser load confirms the RC.35 private bundle, visible keyboard guidance and an empty console without changing the owner's in-progress draft.
...
|---|---|---|
| Application | `0.3.0-rc.35` | `ghcr.io/tinfoyle/spyglass-engine@sha256:a38ba77e9821a0b6756dd722ff3c3f8b1fb8f11baced106699e1ab0e79fd8fce` |
| Public UI | `0.3.0-rc.35` | `ghcr.io/tinfoyle/infinite-ocean-public-ui@sha256:9951905ce4efb811c50c8b6ba853bc70262f75390aab371e65e8ea0aa933c267` |
| Private UI | `0.3.0-rc.35` | `ghcr.io/tinfoyle/infinite-ocean-private-ui@sha256:96f4d457f6222702827c9e51039532519dcd9032bf5339ef0bb1ca8115c5a157` |

The RC.35 triple was built from source revision
`2af23ef1ed18dcf5fff8bd8be84ec2fcee89c89c` and is tracked in
`deploy/releases/0.3.0-rc.35.env`. It passed local AMD64 admission and connected
...
The historical connected-stage RC.5 -> RC.4 -> RC.5 rehearsal completed successfully against retained PostgreSQL volumes. RC.4 and the restored RC.5 each passed the same 11-check exact-origin/Catalog boundary certificate. RC.5 remains the Phase 2.5 platform rollback baseline; RC.35 is active on Stage, and its expanded schema does not inherit that earlier downgrade proof. The final production release will be a later signed immutable triple; no current candidate is predeclared as that release.
...
The same matched application/public/private digests must move through local validation, Hostinger Stage and LKE production configuration without rebuilding. The final operator release path must supply attached SBOM, BuildKit provenance, a trusted signature and retention evidence; the unsigned single-platform RC.35 Stage artifact does not satisfy that final gate.
