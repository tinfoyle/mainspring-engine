...
The Operations Console is locally feature-complete but is not part of Stage RC.31. Do not add it to Hostinger Stage or LKE until the product owner authorizes that separate surface. A future deployment requires a dedicated Operations subdomain/origin, separate passkey-only cookie boundary, five unique least-authority PostgreSQL credentials, offline staff-role provisioning, environment-specific monitoring/backups and completion of the gates in [Operations Console operations](operations-console-operations.md). Its customer, public and staff origins must remain distinct.
...
RC.31 deploys the adaptive `Operations Guide` Baseline compatibility repair from source revision `02758392f8ac169ae0364542143e44ad64ef4fd2` and tracked manifest/checkout revision `746bce1f6c90044eeca99bb438d16283dc965ae0`. The application, public UI and private UI are pinned by digest in `deploy/releases/0.3.0-rc.31.env`, carry BuildKit SBOM/provenance attestations, and passed the publisher's high/critical vulnerability and secret scans with zero findings. The repair preserves empty Agent-result arrays in new backend responses and normalizes historical JSON `null` collections in Vue, preventing the Baseline blank-panel crash. The protected configuration verified before and after deployment. All 48 healthchecked workloads are healthy among 49 running containers; the three retained PostgreSQL ledgers remain at global migration 68 and cell migrations 80/80. Public, login and MCP protected-resource metadata return `200`, and the live application/private UI container identities equal the manifest digests. An authenticated reload of the previously affected assessment visibly serves the RC.31 bundle and renders the interview, reply control and business notebook without a new console error.
...
|---|---|---|
| Application | `0.3.0-rc.31` | `ghcr.io/tinfoyle/spyglass-engine@sha256:dfe07505c11ff584e8b239007186aa729d56e830636a020585c92270ce28a7b8` |
| Public UI | `0.3.0-rc.31` | `ghcr.io/tinfoyle/infinite-ocean-public-ui@sha256:39ead0b97124488e64de46afb9cb945add0580b4b6305c3bee99e6c091ff6da3` |
| Private UI | `0.3.0-rc.31` | `ghcr.io/tinfoyle/infinite-ocean-private-ui@sha256:2cf281f33a917568aff7173579093a995741ccb8b7168e8a872344dcde0567c0` |

The RC.31 triple was built from source revision
`02758392f8ac169ae0364542143e44ad64ef4fd2` and is tracked in
`deploy/releases/0.3.0-rc.31.env`. It passed local AMD64 admission and connected
...
The historical connected-stage RC.5 -> RC.4 -> RC.5 rehearsal completed successfully against retained PostgreSQL volumes. RC.4 and the restored RC.5 each passed the same 11-check exact-origin/Catalog boundary certificate. RC.5 remains the Phase 2.5 platform rollback baseline; RC.31 is active on Stage, and its expanded schema does not inherit that earlier downgrade proof. The final production release will be a later signed immutable triple; no current candidate is predeclared as that release.
...
The same matched application/public/private digests must move through local validation, Hostinger Stage and LKE production configuration without rebuilding. The final operator release path must supply attached SBOM, BuildKit provenance, a trusted signature and retention evidence; the unsigned single-platform RC.31 Stage artifact does not satisfy that final gate.
...
On 2026-08-21 the website and application stage names resolved to the VPS public address and the shared Caddy routes were activated with the repository's shared security-header policy, including HSTS. On 2026-08-22 the MCP wildcard hostname was activated. On 2026-08-27 `/opt/spyglass-stage/current` advanced to the RC.9 checkout and connected browser acceptance confirmed the Your Turn-led Features story, all twelve detail links, canonical metadata and no horizontal overflow. On 2026-09-03 the selector first advanced to RC.30 for the adaptive interview, then to `/opt/spyglass-stage/releases/746bce1f6c90044eeca99bb438d16283dc965ae0` for the RC.31 null-collection compatibility repair. The exact RC.31 application/public/private digest triple is deployed, 49 long-running containers are present, all 48 healthchecked workloads are healthy, the internal edge is running, and the retained global/cell A/cell B PostgreSQL services remain at migrations 68/80/80. The previously blank assessment now renders in the authenticated browser; owner acceptance of the complete adaptive Baseline journey remains open.
