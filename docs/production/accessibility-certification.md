...
1. Add repeatable exact-artifact browser jobs for the Nuxt acquisition routes and Vue signup, verification, login, owner enrollment, Your Turn, Work, Knowledge, Agents, Schedules, Account, Billing, Security, Account exports, Account lifecycle, Privacy, Checkout and Affiliate routes at every required width; cover responsive overflow, target size, focus order, drawer/dialog containment and restoration, skip-link movement, zoom/reflow and browser-computed contrast.
