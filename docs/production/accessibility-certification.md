...
The Nuxt acquisition surface now has route-inventory, consent-state, Catalog-resilience, canonical/structured-discovery and rendered local TLS contracts across all seventeen crawlable routes. The Vue shell has component contracts for drawer semantics, first/last enabled-control focus containment, Escape restoration, route-change focus, modal focus containment/restoration and reversible background isolation. The long mobile navigation list scrolls independently while Account selection remains outside that scrolling region, and standalone Affiliate support controls meet the product's 44-pixel target. These are meaningful regression controls, but real-browser axe/computed-layout coverage and the complete browser/assistive-technology matrix are still required; happy-dom component tests and HTTP inspection are not substitutes.
