...
Current exact-build checkpoint (2026-08-25): 341 browser checks pass with eleven intentional skips on profiles where the compact menu is not rendered. The ten additional checks protect the consequential Your Turn decision form across all ten standard profiles: in-flight submission blocks route changes with a visible status, unsaved decisions invoke the browser's native leave confirmation, canceling the confirmation preserves the exact draft, and accepting it permits navigation. The same screen installs a browser-unload safeguard while protected state exists. Component tests cover the underlying detail states; every browser path also asserts horizontal reflow and browser-computed axe rules. This is deliberately scoped to Your Turn. Safe-navigation certification for the other feature editors and actual 400% browser zoom remain open.
