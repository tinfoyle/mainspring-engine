...
Current exact-build checkpoint (2026-08-25): 301 browser checks pass with eleven intentional skips on profiles where the compact menu is not rendered. The matrix now includes stale-write recovery for exact Knowledge, Agent Persona, Finance and Integration interactions across all ten standard profiles, including authoritative reload, safe local-intent handling, retry availability, overflow and browser-computed axe assertions. That interaction found and corrected an invalid Unicode-set HTML pattern in the Persona editor. This supersedes the earlier 291-check count below; package-domain mutation conflicts are no longer an open certification item.
