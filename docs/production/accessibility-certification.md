...
Current exact-build checkpoint (2026-08-25): 331 browser checks pass with eleven intentional skips on profiles where the compact menu is not rendered. In addition to the conflict, capacity and downstream-recovery paths recorded below, all ten standard profiles now exercise an active Affiliate's aggregate renewal ledger, generated code and customer-identity concealment; then reload the same dashboard under suspension to prove that new sharing fails closed while history and the structured appeal remain available. Checkout separately proves that a self-referral denial retains the deliberately entered code and retryable customer state while analytics consent is denied and no optional event is emitted. Each path includes overflow and browser-computed axe assertions. This supersedes the earlier counts below; implemented Affiliate active/suspended presentation and consent-independent self-referral recovery are no longer open synthetic-browser items.
