...
1. Expand the exact-artifact browser job from its Chromium Your Turn, Checkout, Privacy and Affiliate checkpoint to the Nuxt acquisition routes and Vue signup, verification, login, owner enrollment, Work, Knowledge, Baseline, Agents, Schedules, Finance, Integrations, Account, Billing, Security, Account exports and Account lifecycle routes at every required width; cover target size, forward and backward focus order, every drawer/dialog boundary, skip-link movement, zoom/reflow and forced colors.
2. Expand real-browser axe coverage to both new UI artifacts, Firefox and WebKit, and continue private Vue state coverage through validation, denial, conflict, capacity, loading and recoverable-failure states. Retain the legacy structural gates as rollback evidence, not as evidence for Vue parity.
