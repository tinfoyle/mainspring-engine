...
Current exact-build checkpoint (2026-08-25): 311 browser checks pass with eleven intentional skips on profiles where the compact menu is not rendered. The matrix includes authoritative stale-write recovery for Knowledge, Agent Persona, Finance and Integration interactions; Agent concurrent-run capacity denial; and safe retry-intent preservation after Knowledge, Baseline, Finance, Marketing, Integration and Schedule downstream failures across all ten standard profiles. Each path includes overflow and browser-computed axe assertions. These interactions found and corrected an invalid Unicode-set Persona pattern, transparent modal surfaces that failed contrast while errors kept them open, and Schedule errors rendered outside their modal. This supersedes the earlier counts below; implemented package conflicts and capacity/downstream recovery are no longer open certification items.
