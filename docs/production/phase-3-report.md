...
Document-foundation checkpoint (2026-08-21): [ADR-0004](decisions/0004-knowledge-document-lifecycle.md) selects an S3-compatible object boundary using containerized MinIO in `ubunturojo` and Hostinger stage and Linode Object Storage in LKE production. A typed Account-owned document/revision kernel now fixes immutable source identity and digest, 50 MiB source/8 MiB extraction limits, trusted media agreement, quarantine-first scan/extraction/index states, stable chunks, ready-only publication, retention, legal hold and deletion entry. Four cell tables add forced RLS, immutable revision guards, exact current-revision validation, redacted events, movement fencing and erasure counting. The PostgreSQL repository and application service cover replay-safe admission, workload-only processing, atomic chunk/index completion, human publication, sensitivity-aware reads and retention-checked deletion requests. Unit and disposable-PostgreSQL tests prove processing order, cross-Account denial, metadata immutability and event redaction. A bounded S3 adapter and admission coordinator now authorize before storage, conditionally create immutable exact-size/digest objects, require bucket versioning, verify replay/read metadata, address exact versions and clean newly-created objects when database admission fails. Pinned MinIO/MC services, generated stage secrets, static stage SSE key, versioned bucket initialization and local/CI integration tests establish the Docker path; LKE remains mapped to managed Linode Object Storage. A fail-closed ClamAV client now verifies `INSTREAM` support, streams the exact declared source size in bounded network-order frames, enforces deadlines and bounded replies, and records only clean or validated infected-signature verdicts; tests cover framing, source mismatch and daemon/protocol failures. ClamAV service placement, extraction and the processing worker, routed operations, physical deletion reconciliation and retrieval remain open, so the document bullet below is not complete.
