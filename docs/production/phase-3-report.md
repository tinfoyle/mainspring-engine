...
Document-foundation checkpoint (2026-08-21): [ADR-0004](decisions/0004-knowledge-document-lifecycle.md) selects an S3-compatible object boundary using containerized MinIO in `ubunturojo` and Hostinger stage and Linode Object Storage in LKE production. A typed Account-owned document/revision kernel fixes immutable source and extracted-text object identities and digests, 50 MiB source/8 MiB extraction limits, trusted media agreement, quarantine-first scan/extraction/index states, stable chunks, ready-only publication, retention, legal hold and asynchronous deletion. Forward migrations add exact extracted-object key/version persistence, content-free processing and deletion lease/retry/dead-letter queues, immutable content-free exact-version deletion receipts, movement fencing and exact erasure accounting. Customer metadata remains behind forced Account RLS while shared workers claim identifiers only. The repository/application paths cover replay-safe admission, workload processing, atomic chunk/index completion, human publication, sensitivity-aware reads, retention-checked deletion entry and physical deletion reconciliation. The existing per-cell worker fairly services both queues with the same revision-prefix object credential and exposes separate queue age/dead-letter status. It rechecks deletion eligibility, loads an Account-scoped exact manifest, deletes only required source/extracted versions, persists each receipt, and atomically removes chunks, marks revisions/document deleted, appends a redacted event and completes the lease. Exact-version delete replay converges after unknown object-store commits; missing or mismatched receipts fail closed. The routed boundary provides multipart upload, content-redacted list, exact detail, publication and deletion entry with independent router/cell digest binding and private disk spooling. Unit, race, disposable-PostgreSQL, real MinIO, malware/extraction, generated-contract and stage-config tests cover the completed path. Retrieval and citation validation remain open, so the document/retrieval work below is not complete.
