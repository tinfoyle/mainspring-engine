...
Document-foundation checkpoint (2026-08-21): [ADR-0004](decisions/0004-knowledge-document-lifecycle.md) selects an S3-compatible object boundary using containerized MinIO in `ubunturojo` and Hostinger stage and Linode Object Storage in LKE production. A typed Account-owned document/revision kernel now fixes immutable source and extracted-text object identities and digests, 50 MiB source/8 MiB extraction limits, trusted media agreement, quarantine-first scan/extraction/index states, stable chunks, ready-only publication, retention, legal hold and deletion entry. Forward migrations add exact extracted-object key/version persistence and a content-free lease/retry/dead-letter queue with movement fencing and exact erasure accounting. Four RLS-protected document tables retain immutable revision guards, exact current-revision validation and redacted events. The PostgreSQL repository and application service cover replay-safe admission, workload-only processing, atomic chunk/index completion, human publication, sensitivity-aware reads and retention-checked deletion requests. Unit and disposable-PostgreSQL tests prove queue exclusion/completion, processing order, cross-Account denial, metadata immutability and event redaction. A bounded S3 adapter and admission coordinator authorize before storage, conditionally create immutable exact-size/digest source and extracted objects, require bucket versioning, verify replay/read metadata, address exact versions and clean newly-created objects after failed database recording. Pinned MinIO/MC services, generated stage secrets, static stage SSE key, versioned bucket initialization and local/CI integration tests establish the Docker path; LKE remains mapped to managed Linode Object Storage. A fail-closed ClamAV client and pinned private service verify `INSTREAM` support, stream the exact declared source size in bounded network-order frames, enforce deadlines and bounded replies, and return only clean or validated infected-signature verdicts; live tests exercise both outcomes. Local/CI freeze signatures while stage updates a persistent signature volume. A private Tika client and digest-pinned, read-only, capability-free service enforce the media allowlist, disable embedded extraction, cap parser time/resources/output and produce normalized UTF-8 with a stable digest; real-container tests certify text, PDF and DOCX. A versioned deterministic chunker prefers semantic UTF-8 boundaries, uses bounded overlap and records exact extracted-object byte offsets plus stable lexical token counts. One restart-safe, least-privilege worker per cell resumes from persisted state and exposes dependency-aware readiness plus queue-age/dead-letter status; both local replicas reached healthy against real PostgreSQL, MinIO, ClamAV and Tika. The future stage command enables this `knowledge-processing` profile. Five generated customer operations now route multipart upload, content-redacted stable list, exact processing detail, ready-revision publication and retention-checked deletion entry. Request bodies are independently digest-bound at router and cell while private temporary spools preserve the 50 MiB streaming boundary and retryability. Admission derives size, digest and verified media from the exact uploaded file, checks PDF/DOCX/structured-text signatures and refuses caller assertions for those fields. Full local Docker, race, disposable-PostgreSQL, MinIO, generated-contract and stage-config suites pass. Physical deletion reconciliation and retrieval remain open, so the document bullet below is not complete.
