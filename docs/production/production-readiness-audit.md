...
| Quality | Full Go, race, vet, vulnerability, website build/lint/render, npm production audit, PostgreSQL migration, manifest verification, static scratch-image build, runtime identity, SBOM/provenance, signed-release workflow contracts, bounded content-free read/write evidence tooling, and content-safe OTLP/HTTP tracing in CI | Strong repository evidence; connected release run required |
...
4. **Observability and incident response.** The runtime now publishes bounded-cardinality HTTP counters/durations, content-free worker/HPA metrics, and opt-in bounded OTLP/HTTP traces with HMAC-pseudonymous Account/cell correlation. Deploy and rehearse the collector contract in [observability-operations.md](observability-operations.md); add queue-age and projection-lag dashboards, paging thresholds, ownership, and remaining worker/database/controller spans. Complete a staging burn-in and one game day.
