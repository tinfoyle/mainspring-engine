...
Current local recovery amendment (2026-08-25): the exact-build matrix now supplies 301 applicable passes and eleven intentional profile skips. It additionally proves stale Knowledge decisions, Agent Persona publication, Finance posting and Integration disable commands all reload authoritative state before retry; the prior “remaining package-domain mutation conflicts” item is closed. Capacity/downstream failures, connected identity text enlargement, 400% browser zoom/reflow, provider-backed journeys, physical devices and assistive technologies remain open. The exercise also corrected an invalid Persona-editor HTML pattern and did not use an identity, credential, provider, database or deployed environment.
