...
Current local recovery amendment (2026-08-25): the exact-build matrix supplies 301 applicable passes and eleven intentional profile skips. It additionally proves stale Knowledge decisions, Agent Persona publication, Finance posting and Integration disable commands all reload authoritative state before retry; the prior “remaining package-domain mutation conflicts” item is closed. The connected-local identity matrix now supplies thirteen passes, including 200% text enlargement across all six login, signup, verification and recovery routes; the prior connected-identity text-enlargement item is also closed. Capacity/downstream failures, 400% browser zoom/reflow, provider-backed journeys, physical devices and assistive technologies remain open. The exercise corrected an invalid Persona-editor HTML pattern and did not use an identity, credential, provider, database or deployed environment.
