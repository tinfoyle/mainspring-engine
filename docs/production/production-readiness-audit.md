...
3. Replace the OpenAPI contract's shared success responses with operation-specific status/request/response schemas and add browser E2E fixtures. Route, owner, authentication, Go/TypeScript generation, and Go transport drift checks are executable.
...
- Run many-small-Account, one-hot-Account, queue-backlog, pod-loss, node-loss, database-failover, and provider-degradation tests.
