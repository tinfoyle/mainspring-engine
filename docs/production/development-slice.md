...
- A prototype-informed private Agents workspace at `/app/agents` that consumes only routed Account APIs: package lock/read-only modes, Boardroom selection, immutable Persona-version roster, stable Conversation paging, ordered Message transcripts with structured evidence, and new/follow-up Run submission. The browser never receives cell credentials or chooses placement, and Run controls are absent rather than cosmetically disabled when access is read-only.
