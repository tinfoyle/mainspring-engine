...
4. Complete the trust/deployment side of fair asynchronous runner control before promoting reference manifests. The transport-neutral scheduler, identifier-only durable queue, weighted Account fairness, concurrency fencing, crash-recovery leases, due-time terminal inspection, digest-bound fixed-profile Kubernetes Job launch, terminal capacity release, least-privilege database role contract, and Account-erasure integration are executable. Invocation broker and runner identity, exact cancellation, custom scaling signals, environment-specific RBAC/NetworkPolicy/RuntimeClass, and load evidence remain.
