...
4. Complete the Kubernetes side of fair asynchronous runner control before promoting the reference manifests. The transport-neutral scheduler, identifier-only durable queue, weighted Account fairness, concurrency fencing, crash-recovery leases, terminal capacity release, least-privilege database role contract, and Account-erasure integration are executable. Fixed-profile Job launch, terminal watch/cancellation, invocation broker, custom scaling signals, applied RBAC/NetworkPolicy, and load evidence remain.
