...
- A responsive private application shell for signup, verification, login, Account switching, invitation acceptance, package visibility, the operational overview, and package-gated Work and Agents workspaces.
...
- Executable `account-api`, `billing-worker`, `account-lifecycle-worker`, and `identity-maintenance-worker` process modes with strict environment validation, independent connection caps, graceful shutdown, dependency-aware readiness, and content-free retention status.
...
- Forced-RLS Agents persistence for Boardrooms, Persona identities/versions, Conversations, Runs, ordered plan turns, Invocations, Messages, and immutable customer recovery decisions, with Account-composite relationships, execute-only digest-bound success/failure projection, atomic success-plus-one-message semantics, exact idempotent retry, and complete Account-erasure counting/deletion.
...
- Routed Agents serving for Account-scoped Boardroom creation/listing, immutable Persona publication/listing, stable cursor Conversation listing/detail, ordered mixed human/Persona Message pages, Run start/read, and one immutable recovery decision per failed Run. The global router publishes every exact Agents route under the `agents` package rather than leaving cell-only handlers unreachable. Run admission freezes the routed entitlement version and Boardroom policy version, allocates one immutable human message, snapshots exact Persona versions plus explicitly selected Work/Fact/Baseline/ready-current-Document evidence, derives every model/tool operation identity deterministically, and enforces the Account-local `concurrent_runs` limit before dispatch. Document capture includes every integrity-checked chunk, rechecks restricted sensitivity and fails rather than truncating the 48-KiB context envelope. Ordered plans dispatch only turn one; each successful Persona projection atomically advances and enqueues the next turn so its snapshot includes all preceding contributions, while failure cancels every undispatched downstream turn. Retry admission repeats current package and capacity authorization, creates a new Run containing the failed/canceled cohort, and reuses its frozen evidence, original context watermark and exact PersonaVersion digests. Persona Message reads repeat strict result validation and bind the displayed body to the structured contribution before returning it.
- A prototype-informed private Agents workspace at `/app/agents` that consumes only routed Account APIs: package lock/read-only modes, Boardroom selection, immutable Persona-version roster, stable Conversation paging, ordered Message transcripts with structured evidence, new/follow-up Run submission, and explicit retry-or-accept recovery for failed Runs. The browser never receives cell credentials or chooses placement, and Run/recovery controls are absent rather than cosmetically disabled when access is read-only.
...
- GitHub verification for Go format/test/race/vet, disposable PostgreSQL contracts, vulnerability scanning, Kubernetes reference rendering, public-site build/lint/full dependency audit, a static non-root scratch-image build with linker-bound runtime identity, and exact-binary validation of the observability gateway and Prometheus rules. Runtime HTTP services publish fixed mergeable duration histograms; principal routed/provider clients can export bounded, mTLS-authenticated OTLP/HTTP traces with route templates and HMAC-pseudonymous Account correlation while rejecting baggage and raw destinations. The reference adds a digest-pinned, credential-separated, fail-closed Collector gateway, initial content-free SLO/queue/collector rules, and a dashboard; no connected backend, scrape, paging, burn-in, or game-day evidence exists yet. Separate commit-pinned workflows published the first successful application/website RC.2 pair with AMD64/ARM64 indexes, attached BuildKit SBOM/provenance and keyless Cosign signatures; connected staging, scan evidence and a second rollback pair remain.
- Bounded `load-cert` and `write-cert` drivers for reviewed synthetic many-Account and hot-Account profiles. The GET-only driver provides exact release/plan binding, private per-actor session and Account substitution, weighted deterministic pacing, full planned/scheduled/completed accounting, and per-group latency/error thresholds. The separate Work driver owns generated canonical payloads and deterministic operation UUIDs, repeats every exact body/key, requires identical status/ETag/Location/body identity, measures create/replay latency independently, and emits immutable content-free evidence. Neither has yet been run against an applied staging cluster with the required Kubernetes/database/HPA and durable-row evidence.

The development command is intentionally memory-backed and refuses to start unless `SPYGLASS_ENV=development`. Persistent `account-api`, `app-router`, private `tool-router`, `app-api`, `admission-api`, `route-receipt-worker`, `billing-worker`, `notification-worker`, `entitlement-worker`, `account-lifecycle-worker`, `identity-maintenance-worker`, `work-reconciler`, `baseline-maintenance-worker`, `agent-dispatch-worker`, `agent-projection-worker`, and one-shot `route-canary`/`catalog-admin`/`work-release-admin` modes now exist and fail closed until their workload-specific PostgreSQL, route proof, Stripe, origin, encryption, operator, environment-confirmation, workload TLS, or TLS mail configuration is supplied by the environment.
...
Development registration, recovery, invitation, and verified-contact responses expose purpose-specific development tokens because the local composition deliberately uses capture sinks instead of SMTP. The production composition uses the encrypted notification outbox and SMTP adapter, and refuses to expose these token fields.
...
- Rendered browser journey coverage proves signup → verification/password → password/passkey login surfaces → Account shell, identity security/passkey management and verified-contact gate, Membership lifecycle controls, locked/read-only Work package behavior, entitled Work queue structure, and enabled create controls. A 20-state private DOM contract now enforces skip navigation, one focusable main and `h1`, ordered headings, unique identifiers, valid ARIA references, named landmarks and controls, labeled fields, current-page navigation, and no forced initial focus across identity, verified-contact, owner enrollment, security, Account, Work, Agents, and lifecycle variants. Work selection/loading semantics and dialog focus restoration plus Agents reduced-motion behavior are separately locked by script contracts. API journey coverage proves verified-contact strong-auth denial → request → mailbox proof → all-session revocation → new login with unchanged Account memberships, plus invitation → existing identity → Membership role/suspension/reactivation/ownership/self-leave → Account-list revocation and signed Work read/command contracts. Those development-handler responses now fail tests on OpenAPI status, header, media type, empty-body, or JSON-schema drift, and the public pricing client rejects malformed full-Catalog fixtures before rendering live offers.
- The public website build, rendered-route/security-header tests, lint, and full dependency audit pass. Every public route now also passes a rendered DOM accessibility gate for a stable one-header/one-main/one-footer frame, one `h1`, skip navigation, focusable main target, unique IDs, and the layout-independent axe-core WCAG 2.0/2.1/2.2 A/AA rule set. A controlled local-browser journey additionally exercised synthetic registration, verification, password login, owner enrollment, locked Work, current-navigation state, a 390 CSS-pixel viewport, absence of horizontal overflow, and the corrected 32-pixel Account-switch target. This is development evidence only: browser-computed contrast, zoom/reflow, private-route axe, repeatable exact-artifact automation, and the assistive-technology/device matrix in [Accessibility target and release certification](accessibility-certification.md) remain connected-staging evidence. `cmd/staging-cert` binds a deployed anonymous-boundary check to exact HTTPS origins, Git revision, image digest, Catalog version, packages, and offers while emitting only content-free immutable evidence.
- `cmd/load-cert` exercises only bounded GET traffic and rejects partial runs, ambiguous actor Account mapping, redirects, unreviewed environment references, oversized responses, and mutable evidence. `cmd/write-cert` separately exercises only generated Work creation, repeats every exact operation, rejects response-identity drift, and never accepts a caller-authored route or payload. Unit coverage proves weighted hot/small scheduling, strict credential/Account separation, credential/UUID/path/body non-disclosure, strict plans, and immutable reports. Connected cluster metrics, server-side durable-row/capacity assertions, runner load, failure injection, and soak evidence remain.
- The private website preview is deployed at `https://infinite-ocean-spyglass.tinfoyle.chatgpt.site`.
- Disposable PostgreSQL 17 tests execute all migration sets, verify idempotency and checksum drift rejection through a schema-qualified canonical ledger, exercise network-actor budgets, encrypted Account-attributed notification persistence, verified-contact challenge hashing/old-login continuity/atomic login migration/all-session revocation/replay rejection/Account-membership preservation, attributed provider-event persistence, post-erasure provider-retry suppression behind the shared Account-erasure fence, four-eyes Account-erasure preparation, leased split-role cell/global execution, exact Account deletion with other-Account/User preservation, content-free tombstones, one-time capacity decrement and idempotent completion, fair runner admission with noisy-neighbor concurrency isolation and stale-lease recovery, runner-control erasure/replay coverage, passkey persistence, single-use passkey ceremonies, credential-counter fencing, cross-User isolation, active-plus-retained passkey keys, old-envelope re-encryption and old-key retirement, Account closure billing/freeze/recovery/block/retry/retention invariants, concurrent Catalog version allocation, four-eyes publication, immutable content/mappings, forward and lower-version entitlement rollout, unchanged-access drift repair, independent-grant preservation, governed limit propagation, concurrent capacity admission without oversubscription, UUID retry/release idempotency, expiry reclamation, stale-entitlement rejection, registration, Checkout reservation concurrency, broker-backed Work creation/compensation through split global/cell roles, representative Work query plans over a 20,609-row cell fixture, tie-heavy cursor pagination across four page sizes, 24 simultaneous completion/assignment pairs, direct Account-scoped Work queries, and transaction-local RLS isolation through non-owner roles. Kubernetes resources remain review-only and have not been applied to a cluster.
