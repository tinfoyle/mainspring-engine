...
- OpenAPI 3.1 route ownership and drift enforcement for every customer-facing Account/cell route, with resolved-reference validation; exact public Catalog, registration, password/passkey login, Account-choice/selection, passkey lifecycle, identity recovery, recovery-code, security-posture, session-management, cell Account-context, Work, and Agents query/header/body/status/response contracts; generated Go route maturity; and generated TypeScript route/schema inventories.
...
2. Extend the executable Agents foundation with conversation/message read models, orchestration beyond one ordered Persona pass, schedules, attachments, customer-visible retry/manual resolution, and governed consequential adapters; rehearse the executable audited dispatch/projection dead-letter recovery in staging, then apply the same private admission and reconciliation boundaries to Knowledge, Finance, and Marketing.
