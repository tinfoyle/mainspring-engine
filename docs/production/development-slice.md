...
2. Extend the executable Agents foundation with conversation/message read models, orchestration beyond one ordered Persona pass, schedules, attachments, controlled dispatch/projection dead-letter recovery, and governed consequential adapters; then apply the same private admission and reconciliation boundaries to Knowledge, Finance, and Marketing.
