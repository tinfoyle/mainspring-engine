...
3. A credential binding names an opaque secret-broker reference and generation. Only the reference's SHA-256 attestation is stored in PostgreSQL. The execute-only claim returns that attestation and the non-secret provider code to the dedicated connector runtime; its broker must prove an exact Account/credential/generation/provider/reference match before leasing material. Provider credentials are never returned by an API, written to an event, supplied to an Agent, or stored in Marketing. Rotation creates a new generation; revocation is monotonic. The connector runtime resolves the reference only for one bounded operation.
...
- Docker and Kubernetes may use the reloadable mounted broker over a read-only secret volume; a managed secret-store CSI driver can back the same filesystem contract. Ordinary app and runner pods receive neither the mount nor broker authority.
...
- Local Docker uses the mock connector to inject definitive failure, retryable failure, ambiguous acceptance and reconciliation outcomes without external credentials. Stage will repeat the same cases through approved provider fixtures before its connector workers can open.
- Mounted-broker tests prove exact reference-attestation/provider binding, hot index rotation, bounded material, restrictive file modes and immediate lease zeroing. Fresh PostgreSQL tests prove the execute-only claim returns the exact stored provider and attestation without granting direct credential-table reads.
