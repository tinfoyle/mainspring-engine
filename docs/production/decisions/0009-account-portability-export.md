...
8. Download authorization returns a short-lived, one-artifact capability only after current Account role, recent passkey evidence and export-request state are checked. HTTP and browser sessions use the current session evidence. MCP OAuth captures the approving passkey timestamp once; refresh preserves it without renewal. Capabilities are accepted only in the custom Authorization header, never in a URL. The durable export reference is not exposed in ordinary status, logs, metrics, browser storage or events. Artifact expiry is no more than 30 days and drives idempotent physical deletion.
...
- The artifact builder, schema registry, durable request/expiry state, exact-version object publisher/deleter, private staging/pipeline kernel, two-database repeatable-read coordinator, every global/cell database projection and the exact Knowledge/Marketing source factory feed one composed worker and customer surface. HTTP, browser and MCP adapters reuse the same Owner/strong-auth application services. Applied environment certification remains a separate release gate.
...
- HTTP, browser and MCP tests prove Owner-only request/history/status/cancellation, immutable-state-bound download capabilities, no query-string token path, no artifact-reference/digest disclosure and non-renewable OAuth passkey evidence.
