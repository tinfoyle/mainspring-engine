...
3. **Isolation and load.** Use the bounded read driver and evidence procedure in [load-certification.md](load-certification.md) to run many-small-Account and one-hot-Account profiles through at least two router and cell replicas. Record latency histograms, errors, queue ages, database connections, HPA decisions, and fairness. Include wrong-Account, wrong-cell, stale-placement, stale-entitlement, replay, and downstream-saturation attacks. The content-free client report does not replace the coincident cluster evidence.
...
6. **Accessibility and responsive certification.** Run automated WCAG checks plus keyboard, focus, screen-reader, zoom, reduced-motion, contrast, error-recovery, and supported-device reviews across public acquisition and private identity/Account/Work journeys. Automated lint alone is not conformance evidence.
