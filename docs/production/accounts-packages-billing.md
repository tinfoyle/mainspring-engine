...
Approved AI usage unit (2026-08-26): Infinite Ocean AI Tokens are customer-facing, provider-neutral usage credits, not raw provider tokens, money, cryptocurrency or a promise of access to one named model. Each admitted model operation freezes an immutable customer rate-card version that converts its model/provider class, input, cached input, output and billable tool use into AI Tokens. The system reserves a bounded maximum before provider work, settles exact trusted usage afterward, releases unused reservation once, and cannot produce a negative balance or retroactively reprice completed use. New rate cards affect future admissions only. This abstraction permits reviewed Kimi, Z.ai or later model substitutions while preserving the published customer allotment and Infinite Ocean margin. The included monthly allotment, package sizes/prices, carryover and transfer/refund rules remain separate decisions.

Approved model presentation (2026-08-26): launch exposes one provider-neutral **Standard AI** service. Customers do not select or receive a commercial promise for Kimi, Z.ai or a specific model identifier. Infinite Ocean may change the underlying provider/model only after the candidate passes the same local quality, structured-output/tool-use, privacy/no-storage, reliability, usage-normalization and cost gates and enters a reviewed prospective rate card. Every admitted Run freezes its internal provider, model, adapter, policy and rate versions so a later substitution cannot alter retries, reconciliation, evidence or customer debits. Additional customer-facing model classes are post-launch decisions.
