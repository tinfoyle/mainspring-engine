...
Human MCP drafts receive human provenance and reject a supplied Run. A workload draft is accepted only when its authenticated identity has the exact `runner-invocation:<uuid>` form and the caller supplies a valid Run; the adapter derives the invocation identity rather than trusting it as tool input. Application policy still rejects workload revision, submission, approval, activation, cancellation and campaign governance. The asset tool accepts canonical base64 content up to 512 KiB and sends it through the same governed object admission service as HTTP; it cannot accept a content reference, digest or byte count. Asset hashes in output remain lowercase hex, list cursors are versioned and collection-bound, and errors expose only stable safe codes. Protocol tests prove the complete deterministic schema surface, bounded canonical decoding, global routing classification, routed Marketing claims, opaque cursors, human and Agent provenance, version requirements and backend-error redaction.

## Private workspace checkpoint

`/app/marketing` plus durable campaign and release URLs now provide the selected Account a mobile-first Vue workspace over the generated Marketing HTTP boundary. Enabled participants can create and revise campaign intent, append immutable creative files and freeze exact campaign-version/channel/revision release snapshots; Owner/Administrator controls govern cancellation and campaign state. Submitted activation work leads to Your Turn, and the browser never asks a User to paste an internal Approval identity. Read-only Accounts retain campaign, asset and release inspection while every mutation control is omitted. The shell keeps activation distinct from external delivery and does not hold provider credentials, storage references, computed digests or creative bodies in persistent browser state.

Every existing-resource command uses the freshly loaded aggregate version in a weak `If-Match` precondition, and an exact failed retry retains its route-bound idempotency UUID. The creative form selects a file and sends multipart content; internal reference, digest and size fields are absent, images require alternative text, and the release builder selects exact immutable revisions instead of accepting an unstructured identifier list. Customer values render through Vue text bindings and no session, customer or draft material is placed in browser storage. API-client and component tests prove durable navigation, role/package/read-only gating, lifecycle coverage, version/idempotency preconditions, governed file admission and exact snapshot construction. Applied supported-device, browser and assistive-technology evidence remains part of UI7.
...
The dedicated payload source is now constructed through the provider-neutral boundary. It reloads the exact approved release and immutable revisions under Account RLS, verifies every object byte count and SHA-256, reproduces the frozen manifest and emits a bounded versioned connector envelope without exposing opaque storage references. Local certification uses an immutable no-network reader. The versioned S3/MinIO reader and governed admission kernel are constructed and every customer, MCP and Agent creation path now reaches that kernel. The application service independently rejects any non-canonical object-version reference, so new durable revisions cannot introduce arbitrary external references.
...
The production object boundary now derives every key from the exact Account, campaign, asset and revision identities. PostgreSQL retains only a canonical URL-safe encoding of the immutable object version; callers cannot choose a bucket or key. Admission authorizes and validates the revision before upload, computes size and SHA-256 over a seekable body, writes with create-only semantics and removes a newly created orphan if durable revision creation fails. Exact replay reuses the existing version. The Marketing content limit is now 16 MiB in both the domain and migration 66, matching the connector envelope ceiling.

The shared versioned S3/MinIO adapter writes Account/kind/digest metadata and server-side encryption, then the connector reader derives the key again and requires the exact stored version, byte count, digest, Account and `marketing-asset` kind before returning a stream. App-api object policy now permits only the derived Marketing key family in addition to Knowledge source objects; the document worker remains denied Marketing reads and writes. Unit, real MinIO and policy tests pass. App-api now composes this admission path, the routed HTTP contract is multipart, the router grants only this exact route the 16 MiB-plus-envelope limit and upload timeout, and the private form no longer asks for internal object metadata. MCP uses bounded canonical base64; Agent drafting is bounded UTF-8 copy only. Both share admission and neither accepts object metadata.

The Integration connector now has a fourth independent object credential and MinIO policy. It may inspect bucket versioning and read only exact Marketing content versions; it cannot list the bucket, read Knowledge objects, write or delete. Compose reserves this identity only for the connector workers, Stage secret preparation generates/preserves it independently, and the process inventory declares the object-store dependency. A listing-free `VerifyReadOnly` startup contract and a real MinIO certificate prove app write, connector read and every negative boundary. The production worker now consumes that identity directly and fails startup if versioning or exact-read authority is unavailable.
