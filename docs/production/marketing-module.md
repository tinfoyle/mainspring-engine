...
Marketing remains non-executable because production creative admission is not yet exposed through the customer/Agent transports and no real provider adapter is approved. The manifest source, one-operation lease boundary, worker bootstrap and local Docker certification are constructed. No generic app or Agent runner receives provider authority.
...
The dedicated payload source is now constructed through the provider-neutral boundary. It reloads the exact approved release and immutable revisions under Account RLS, verifies every object byte count and SHA-256, reproduces the frozen manifest and emits a bounded versioned connector envelope without exposing opaque storage references. Local certification uses an immutable no-network reader. The versioned S3/MinIO reader and governed admission kernel are now constructed, but customer/Agent creation still uses the prior metadata contract until the next transport slice replaces it; arbitrary external references are not considered executable.
...
A dedicated per-cell worker bootstrap now composes the global authority, cell execution store, payload assembler and injected connector-only dependencies with bounded pools and health/status. An opt-in, restore-gated ubunturojo Docker profile certifies this path end to end with a local-only no-network mock, an exact approved release and least-privilege split database roles. This is certification infrastructure rather than a customer Marketing surface: object admission transport/composition, mounted-broker composition and real provider adapters remain prerequisites, so Marketing remains non-executable.
