...
Privacy-rights deadline amendment (2026-08-26): the response calendar month clamps to the target month's last valid day, eliminating the January-31-to-March normalization error. The least-authority operator role can now discover at most 100 open requests through an explicitly cutoff-bound `list-open` function. Results contain only opaque request workflow metadata, the listing creates immutable access evidence, and direct request/audit table access remains denied. Identity-bearing inspection and evidence-bound resolution remain distinct authorized operations.
