...
- Status: GDPR-capable analytics scope and Affiliate product policy approved for Phase 3; implementation, legal and operational release gates remain closed
...
Current policy supersession (2026-08-26): the owner has now approved the economics, adverse-event rules and account-credit-first Support-check settlement model and delegated the aligned remaining AI Token policy. Earlier checkpoint language calling those choices candidate, unresolved or awaiting commercial approval is historical. Enrollment and attribution remain closed because the exact `account_credit_with_support_check` mode, Catalog/Stripe mappings, token implementation, legal/vendor/transfer review and operating procedure are not yet release-complete.
