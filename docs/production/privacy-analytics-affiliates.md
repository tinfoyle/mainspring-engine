...
Launch presents one provider-neutral Standard AI service and does not expose a selectable Kimi, Z.ai or model-specific product. Infinite Ocean may substitute the internal provider/model only after reviewed quality, tool-use, privacy/no-storage, reliability, normalized-usage and cost certification. Each admitted Run remains bound to its original internal execution and AI Token rate versions; privacy notices and the subprocessor record still disclose applicable provider processing as legally required even though product marketing remains model-neutral.
