...
`internal/transport/mcpapi` is a thin transport over the same application services used by customer HTTP. Its base publishes 15 typed tools covering queue, detail, create, answer, decision and cancellation operations for Information requests, Work reviews and consequential approvals. Composing `WithActionRecovery` adds four Owner/Administrator tools for content-redacted action list/detail, manual-outcome request and independent confirmation.
