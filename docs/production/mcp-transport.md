...
- Status: protocol, complete typed tool surface, signed global-to-cell routing, OAuth 2.1 core, distributed endpoint budgets, User grant control, and local/Stage/LKE deployment topology implemented; applied Stage conformance pending
- Phase: 3.1 / 3.6
...
1. complete malformed-protocol, OAuth conformance, routing retry, cancellation, load and multi-replica tests;
2. apply the new global migrations and least-privilege role to Stage, publish the Hostinger MCP ingress, and capture an external-client authorization/tool-call/revocation certificate;
