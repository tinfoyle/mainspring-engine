...
| P2.5.6 Hostinger stage | In progress | VPS inventory, digest-only Compose, release/secret separation, TLS-workload boundaries and coexistence with the existing Infinite Ocean Caddy edge are revision-controlled. RC.2 supplies the first signed digest pair. A clean detached checkout of `63070480fefd8b5909229aefd8223e8eb4091acf` now exists under `/opt/spyglass-stage/releases/` and the empty secrets directory is mode 700. Both stage DNS names, real stage secrets/certificates, edge-owner integration, deployment and connected journeys remain. |
