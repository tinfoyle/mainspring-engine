...
The RC.32 application, public Nuxt UI and private Vue UI images run at the exact digests recorded in `deploy/releases/0.3.0-rc.32.env`; all three were built from application source revision `22df0ef272bb68fcd17b555a4914ec4e84671a61`, include provenance and SBOM attestations, and passed high/critical vulnerability plus secret scanning with zero findings. RC.32 retains the RC.31 null-collection compatibility repair and moves the structured next question into the Operations Guide chat bubble whenever the provider's prose omits it; the human reply field is empty and uses only neutral input guidance. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The three PostgreSQL services retained their persistent volumes at global migration 68 and cell migrations 80/80. The public and login origins and MCP protected-resource metadata return `200`, and the running application/private UI containers match the RC.32 digests. An authenticated reload confirms the question in chat, the empty human-only reply field with `Type your answer here…`, and no browser-console errors. Authenticated owner review of the full adaptive interview remains open. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.32.env)"
