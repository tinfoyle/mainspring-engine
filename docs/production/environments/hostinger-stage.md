...
SMS delivery is not yet configured on Stage. Before publishing the code-method setup wizard there, the protected provider input must supply `SPYGLASS_SMS_GATEWAY_URL`, `SPYGLASS_SMS_GATEWAY_BEARER_TOKEN`, and `SPYGLASS_SMS_FROM`, and the release rehearsal must prove receipt, expiry, replay rejection, request throttling, and provider failure behavior. These values belong only to the notification worker and must not be committed to a release environment file.

The RC.13 application, public Nuxt UI and private Vue UI images run at the exact digests recorded in `deploy/releases/0.3.0-rc.13.env`; all three were built from application source revision `0d7967c6fc90a0f7cab417127b66a32c796130bc`. RC.13 retains collision-safe Google signup and adds a first-login setup wizard that resumes until complete, passkey/recovery-code setup guidance, and a friendly customer sign-out control in the desktop sidebar and mobile menu. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The three PostgreSQL services retain their persistent volumes at global migration 66 and cell migrations 80/80. Live acceptance confirms HTTP 200 responses for the public site, login, signup and direct `/app/setup` route; the deployed Vue bundle contains the setup and sign-out interface; and a sessionless logout request returns HTTP 204.
...
release_file="$(realpath ../../releases/0.3.0-rc.13.env)"
