...
The RC.33 application, public Nuxt UI and private Vue UI images run at the exact digests recorded in `deploy/releases/0.3.0-rc.33.env`; all three were built from application source revision `b84be1dc5b1a79d2947c2bca2d57d64d4deae2a7`, include provenance and SBOM attestations, and passed high/critical vulnerability plus secret scanning with zero findings. RC.33 retains the prior Baseline compatibility and prompt-placement repairs, projects a human reply into chat immediately while the Agent is working, and renders automation acceptance as a compact approval event rather than raw prompt text. The application router now admits the existing Work conversation/provenance link commands; reopening the interview idempotently repaired two setup tasks that the former 404 had left unlinked, without duplication. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The three PostgreSQL services retained their persistent volumes at global migration 68 and cell migrations 80/80. The public and login origins and MCP protected-resource metadata return `200`, and the running application/private UI containers match the RC.33 digests. An authenticated reload confirms two compact approval events, two conversation-linked setup Work items, no route error and no browser-console errors. Authenticated owner review of the remaining adaptive interview remains open. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.33.env)"
