...
The RC.45 image triple is recorded in `deploy/releases/0.3.0-rc.45.env` and was built from source revision `3bcb3bdf2dd60665296134312a2bf8604292cf42`. Each image carries provenance and SBOM attestations and passed high/critical vulnerability plus secret scanning with zero findings. The [functional audit](../stage-functional-audit-2026-09-04.md) records repaired schedule history, agent configuration and summaries, Finance evidence entry, Marketing content review and approval, Knowledge navigation, and plain-language UI changes. Live retests passed for the synthetic workflows. RC.45 additionally clarifies pricing with one monthly-plan card, a separate optional setup section and a plain explanation of AI Token usage and top-ups; desktop/phone accessibility and signup checks passed. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The retained PostgreSQL volumes are at global migration 68 and cell migrations 86/86. The public and login origins return `200`, and the running application/public/private containers match the RC.45 digests. The two existing business questions remain open for real owner input. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.45.env)"
