...
## Verified host facts and live state (2026-08-31)
...
Stage SMS delivery uses the Infinite Ocean Telnyx messaging profile and its assigned US long-code number. The protected provider input supplies `SPYGLASS_TELNYX_API_KEY` and `SPYGLASS_TELNYX_FROM`; these values belong only to the healthy notification worker and are not committed to a release environment file. Configuration, provider ownership and the deployed consent copy are verified. Real-handset receipt and redemption remain the final operator acceptance check.

The RC.15 application, public Nuxt UI and private Vue UI images run at the exact digests recorded in `deploy/releases/0.3.0-rc.15.env`; all three were built from application source revision `77bd3b550a7e610937e2d904245344ff555aa0a8`, include provenance and SBOM attestations, and passed high/critical vulnerability plus secret scanning with zero findings. RC.15 retains collision-safe Google signup and the resumable first-login setup wizard, and adds direct Telnyx API v2 delivery plus the required SMS consent disclosure. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The three PostgreSQL services retain their persistent volumes at global migration 67 and cell migrations 80/80. Live acceptance confirms HTTP 200 responses for the public site and login, the deployed Vue bundle contains the SMS consent text, and the notification worker is healthy with only the Telnyx-specific SMS configuration names.
...
release_file="$(realpath ../../releases/0.3.0-rc.15.env)"
