...
## Verified host facts and live state (2026-09-01)
...
The RC.16 application, public Nuxt UI and private Vue UI images run at the exact digests recorded in `deploy/releases/0.3.0-rc.16.env`; all three were built from application source revision `d86582be0304afa783aca72fd49149678a32af0a`, include provenance and SBOM attestations, and passed high/critical vulnerability plus secret scanning with zero findings. RC.16 retains collision-safe Google signup, the resumable first-login setup wizard and direct Telnyx API v2 delivery. It adds an independent unchecked SMS-consent control, immutable versioned consent receipts without plaintext phone numbers, US-number enforcement, compliant security-code message copy and a public review-evidence page. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The three PostgreSQL services retain their persistent volumes at global migration 68 and cell migrations 80/80. Live acceptance confirms the public consent evidence at `https://stage.infiniteocean.net/sms-consent`, the application login origin, the consent-receipt table and the healthy RC.16 workloads. The production legal-site evidence is published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.16.env)"
