...
## Verified host facts and live state (2026-09-04)
...
The RC.36 image triple is recorded in `deploy/releases/0.3.0-rc.36.env` and was built from source revision `803def5a9d2480c0fba895fc5a8d779834274134`. Each image carries provenance and SBOM attestations and passed high/critical vulnerability plus secret scanning with zero findings. RC.36 retains the earlier Baseline interaction, routing, reconciliation and keyboard repairs; aligns the provider-facing structured-output schema with the domain validator; exposes content-free invocation failure codes; preserves the originating run on user messages; automatically retries malformed model output twice; and leaves an explicit, resumable manual retry after exhaustion without duplicating the owner's answer. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The retained PostgreSQL volumes remain at global migration 68 and cell migrations 80/80. All three public origins return `200`, and the running application/public/private containers match the RC.36 digests. Authenticated browser acceptance reopened the historical 03:38 failed turn, created a retry without duplicating the answer, received the agent response at 04:27 and advanced that Business Baseline to established with its prior Knowledge and Work intact. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.34.env)"
