...
The RC.29 application, public Nuxt UI and private Vue UI images run at the exact digests recorded in `deploy/releases/0.3.0-rc.29.env`; all three were built from application source revision `0867ebfcbc716294c045c53b1d2e95bc8ef2b6a2`, include provenance and SBOM attestations, and passed high/critical vulnerability plus secret scanning with zero findings. RC.29 retains collision-safe Google signup, the resumable first-login security wizard, direct Telnyx API v2 delivery and the reviewed SMS-consent boundary. It additionally presents Business Baseline as a plain-language Spyglass-guided interview while preserving attributable owner evidence and explicit plan approval behind the interface. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The three PostgreSQL services retain their persistent volumes at global migration 68 and cell migrations 80/80. Live authenticated acceptance confirms the RC.29 Baseline prompt and three conversational choices; the public, login and durable Baseline origins each return `200`. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.29.env)"
