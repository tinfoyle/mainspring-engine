...
The RC.35 image triple is recorded in `deploy/releases/0.3.0-rc.35.env` and was built from source revision `2af23ef1ed18dcf5fff8bd8be84ec2fcee89c89c`. Each image carries provenance and SBOM attestations and passed high/critical vulnerability plus secret scanning with zero findings. RC.35 retains the earlier Baseline interaction, routing and reconciliation repairs, and makes the composer follow conventional chat keyboard behavior: Enter sends, Shift+Enter inserts a new line, an active Agent run cannot be double-submitted, and input-method composition cannot accidentally send. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The retained PostgreSQL volumes remain at global migration 68 and cell migrations 80/80. All three public origins return `200`, and the running application/public/private containers match the RC.35 digests. A clean authenticated browser load confirms the RC.35 private bundle, visible keyboard guidance and no browser-console errors; the owner's existing draft in the original review tab was left untouched. Authenticated owner review of the remaining adaptive interview remains open. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
