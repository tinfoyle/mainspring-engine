...
Stage SMS delivery uses the Infinite Ocean Telnyx messaging profile and its assigned US long-code number. The protected provider input must supply `SPYGLASS_TELNYX_API_KEY` and `SPYGLASS_TELNYX_FROM`; these values belong only to the notification worker and must not be committed to a release environment file. Release rehearsal must prove receipt, expiry, replay rejection, request throttling, and provider failure behavior.
