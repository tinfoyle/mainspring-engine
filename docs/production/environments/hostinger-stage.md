...
The RC.31 application, public Nuxt UI and private Vue UI images run at the exact digests recorded in `deploy/releases/0.3.0-rc.31.env`; all three were built from application source revision `02758392f8ac169ae0364542143e44ad64ef4fd2`, include provenance and SBOM attestations, and passed high/critical vulnerability plus secret scanning with zero findings. RC.31 retains the adaptive `Operations Guide` interview and repairs compatibility with existing assessments whose empty Agent-result collections were persisted as JSON `null`: the backend now preserves empty arrays and the Vue boundary normalizes historical null collections before rendering. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The three PostgreSQL services retained their persistent volumes at global migration 68 and cell migrations 80/80. The public and login origins and MCP protected-resource metadata return `200`, and the running application/private UI containers match the RC.31 digests. An authenticated reload of the previously affected assessment renders the interview, reply control and business notebook from `index-Cs18781E.js` without a new browser error. Authenticated owner review of the full adaptive interview remains open. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.31.env)"
