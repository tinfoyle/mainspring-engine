...
The RC.41 image triple is recorded in `deploy/releases/0.3.0-rc.41.env` and was built from source revision `9e8cd343af2696c35006c1a363338014b66aa3a0`. Each image carries provenance and SBOM attestations and passed high/critical vulnerability plus secret scanning with zero findings. RC.41 retains the Baseline interaction and recovery repairs and completes the agent-owned Work lifecycle: eligible Work dispatches automatically, capacity pressure defers without consuming a retry, genuine execution failures retry twice, successful runs either complete the Work or open a specific Your Turn request, and three exhausted executions open a recovery question instead of leaving an unexplained waiting item. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The retained PostgreSQL volumes remain at global migration 68 and cell migrations 85/85. The public and application origins return `200`, and the running application/public/private containers match the RC.41 digests. Live acceptance found the pricing review waiting on a concrete token/cost question and repaired the exhausted launch-readiness review into a separate open Your Turn request. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
...
release_file="$(realpath ../../releases/0.3.0-rc.41.env)"
