...
The RC.34 image triple is recorded in `deploy/releases/0.3.0-rc.34.env` and was built from source revision `d899e2994d688af02e061593f211cff1af794b66`. Each image carries provenance and SBOM attestations and passed high/critical vulnerability plus secret scanning with zero findings. RC.34 retains the earlier Baseline interaction and router repairs, stops replaying completed approved-Work creation during later replies, recovers safely from a concurrent creation, and reconciles a durable reply before setup-Work repair can fail. There are 49 long-running Spyglass containers: all 48 healthchecked workloads are healthy and the internal edge is running without a healthcheck. The retained PostgreSQL volumes remain at global migration 68 and cell migrations 80/80. All three public origins return `200`, and the running application/public/private containers match the RC.34 digests. An authenticated reload confirms one durable copy of the affected reply, two conversation-linked setup Work items, no stale Work conflict and no browser-console errors. Authenticated owner review of the remaining adaptive interview remains open. The production legal-site evidence remains published from the revision-controlled page at `https://www.infiniteocean.net/sms-consent/`.
