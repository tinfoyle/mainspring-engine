...
- The implemented API client needs only `create`, `get`, and `delete` on Jobs in the dedicated cell runner namespace; it must never gain list/watch, Secret reads, pod exec, or wildcard RBAC.
- NetworkPolicy denies all by default in both application and runner namespaces. Runner Pods receive only DNS and cross-namespace broker egress; model/tool/provider paths remain behind the broker and model gateway rather than being granted directly to runners.
