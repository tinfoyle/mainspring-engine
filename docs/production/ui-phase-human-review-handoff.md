...
- **Owner commercial policy is complete.** The local monthly Catalog, private Price mapping model, AI Token ledger, Agent-run admission/settlement, token commerce, promotion, commissioning and account-credit-with-Support-check Affiliate settlement boundaries are encoded. Affiliate restricted retention/minimization, subscription remediation, legal review, privacy-controller facts and operational release fields remain implementation or external gates rather than unanswered product-policy choices.
