...
- Audited implementation snapshot: the exact revision containing this document on `main`
- Review date: unset
- Reviewer: unset
...
Implementation update (2026-08-27): the approved `$50`/no-Free launch Catalog identities, inactive zero-entitlement registration, configurable token Catalog, persistent AI Token ledger, paid-renewal grant projector, safe balance API and first-class provider-neutral Persona complexity contract are implemented and locally verified. Customer APIs and portability artifacts no longer expose private provider/model targeting. Agent admission freezes the exact private execution target and immutable rate, reserves before provider contact, settles trusted normalized usage exactly once and releases failed or unstarted work without repricing retries. Strong-authenticated one-time top-up Checkout, frozen-quantity issuance, unused-remainder Refund reversal, bounded promotion redemption, optional commissioning in initial or later Checkout, and durable duplicate suppression are constructed locally. Billing now exposes those completed boundaries through mobile-first purchase, promotion and later-commissioning controls; Checkout separately confirms recurring and one-time charges, applicable tax and deliberately applied Affiliate attribution. The account-credit-with-Support-check Affiliate settlement mechanism is also complete locally, including policy-frozen reservations, provider compensation and Support-check recovery after adversity. Affiliate verified-erasure restriction, scoped legal holds, seven-calendar-year graph minimization and permanent identity-free code non-reuse are complete locally behind a separate execute-only worker. Subscription remediation is also complete: signed projection owns failure/cancellation timing, staged notices, read-only and restricted access, recovery, day-30 provider termination and the existing guarded closure/erasure handoff. A final zero-data Compose bootstrap publishes the revision-controlled local launch Catalog v3, proves deterministic fixture-only Price mappings, starts every persistent service healthy and checks the new Admission and subscription-lifecycle grants with positive and negative least-privilege assertions. The public pricing page now sources the commissioning amount from that Catalog and routes customers into the implemented initial Checkout while explaining the later Billing option; the public Affiliate terms now publish the approved economics, settlement, lifecycle and retention boundaries while keeping enrollment explicitly closed. Planned local feature construction is complete; human review remains open, the review record remains unsigned and every remote environment remains untouched.
...
| Rendered acquisition site, feature inventory, pricing and Checkout handoff | Seventeen Nuxt-rendered public routes have canonical, metadata, structured-data, link, accessibility and bundle-budget gates. Pricing renders the recurring `$50/month` offer and Catalog-backed optional `$250` commissioning with the implemented initial-Checkout/later-Billing boundary. Checkout retains an explicitly applied referral independently of analytics consent and distinguishes recurring cost, one-time cost and applicable tax. |
| Billing commerce controls | Owners and billing administrators can buy the configured AI Token bundle, redeem a bounded promotion and buy later commissioning once per team. Stable retries, strong-authentication handoff, durable balance/ownership refresh, access remediation and customer-safe success/failure copy are covered across the standard browser profiles. |
...
| Affiliate customer flow | An ordinary User can enroll only when the program is opened under an approved mode, receive and replace a generated code, share a proposal-only link, accept terms, inspect identity-free aggregate/monthly history, export identity-owned data and open structured appeals or reviews. After verified erasure, the same surface exposes only a safe restricted-retention notice and privacy-history link. |
| Affiliate commercial integrity | Explicit Checkout application, multi-Account self-referral denial, immutable attribution, replay-safe rolling recurring earnings, append-only Refund/lost-dispute/eligible-credit-note reversals, adverse-before-earning reconciliation, Account-credit application, Support-check accounting, provider compensation and customer-identity concealment are covered locally. Public terms accurately disclose the approved `$10` full-price earning, proportional adjustments, `$100` check-review threshold and seven-year retention while enrollment remains closed. |
| Subscription remediation | First-failure and paid-term clocks, days 0–7 read-only access, Account restriction, immediate/day-7/day-23/day-29 notices, recovery, provider termination and guarded closure handoff are durable and replay-safe. The SPA exposes only recovery-safe routes while restricted. |
| Operator boundaries | Privacy fulfillment, Affiliate lifecycle/retention, Affiliate support, Affiliate deadline minimization and analytics reporting use separate audited execute-only database roles and do not grant direct table access. |
| Clean local bootstrap | A separate disposable Compose project was deleted with only its own named volumes, recreated from zero, and passed the certificate-verified local smoke gate. Admission, Billing, Notification and Account-lifecycle workers remained at zero failures with the Catalog v3 and least-privilege assertions active; the existing developer volumes were preserved. |

The exact UbuntuRojo Docker gate for the snapshot passed 413 applicable browser checks with eleven intentional profile-specific skips (424 total), 79 private UI tests, 22 public tests, 44 API-client tests, all seventeen rendered public routes, type checking, lint, both production builds, gzip budgets and a high-severity dependency audit with zero findings. The current private/public JavaScript measured 118.2/91.9 KiB gzip and CSS measured 10.8/4.7 KiB gzip. A rebuilt local Docker website was then inspected visually at the public pricing and Affiliate-terms routes; both retained clear hierarchy and readable content, and no remote system was contacted.
...
- **Decided 2026-08-26:** launch pricing is `$50.00 USD` (`5,000` minor units) per month per team plus an optional one-time `$250.00 USD` (`25,000` minor units) onboarding/commissioning package, before applicable Stripe-calculated tax. The effective local Catalog encodes both offers; the private Price-mapping model and deterministic local fixtures are complete, while real provider Price identities remain deployment-specific release inputs;
...
- **Decided 2026-08-26:** meter customer model usage in provider-neutral Infinite Ocean AI Tokens, not raw vendor tokens or currency. Each operation freezes a versioned rate, reserves before provider work and settles trusted actual usage exactly once; provider/model substitutions and later rates never reprice history;
- **Corrected 2026-08-26:** do not expose provider/model names. Give each Agent five labeled complexity stops—Simple, Efficient, Balanced, Thorough and Advanced—with Balanced as the default. Show the estimated token range accessibly; Runs freeze complexity, execution and rate versions;
- **Decided 2026-08-26:** subscription token grants, top-up bundle quantities/prices and promotions are independently versioned prospective Catalog records, not a permanent exchange rate. Seed construction at 10,000 tokens per renewal and 10,000/$10, but require release reaffirmation and permit later reviewed pricing/quantity changes;
- **Decided 2026-08-26:** included Tokens expire without rollover when the next successful renewal grant posts; purchased Tokens do not expire while the Account is active; promotions have explicit expiry. Consume earliest-expiring cohorts first and preserve purchased Tokens until last;
...
- **Owner commercial policy is complete.** The local monthly Catalog, private Price mapping model, AI Token ledger, Agent-run admission/settlement, token commerce, promotion, commissioning, account-credit-with-Support-check Affiliate settlement and restricted-retention/minimization boundaries are encoded. Subscription remediation, legal review, privacy-controller facts and operational release fields remain implementation or external gates rather than unanswered product-policy choices.
...
AI usage unit: provider-neutral Infinite Ocean AI Tokens with immutable prospective rate cards and reserve/settle ledger; vendor/model substitution allowed (owner decision 2026-08-26)
AI presentation: provider/model names hidden; five Agent complexity stops (Simple/Efficient/Balanced/Thorough/Advanced), Balanced default, accessible estimates; Runs freeze versions (owner correction 2026-08-26)
AI token commerce: versioned configurable subscription grants, bundles and promotions; local seed 10,000/renewal and 10,000/$10, release-reaffirmed (owner decision 2026-08-26)
...
1. Complete the owner product/visual review against the exact committed local artifacts, one decision at a time, without enabling any feature flag.
2. Record the approval or review-blocking defects in this handoff; resolve any blocking defect locally and rerun the proportionate gate.
3. Complete the provider, legal and physical-accessibility certificates required for release.
4. Re-audit the accepted committed artifact and freeze its deployment-specific configuration with Affiliate enrollment and attribution still fail-closed until their release gates close.
5. Only then hand immutable application and website digests to the separate Stage/production deployment plan.
