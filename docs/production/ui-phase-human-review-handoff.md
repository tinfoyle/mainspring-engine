...
Implementation update (2026-08-26): the approved `$50`/no-Free launch Catalog identities, inactive zero-entitlement registration, configurable token Catalog, persistent AI Token ledger, paid-renewal grant projector, safe balance API and first-class provider-neutral Persona complexity contract are now implemented and locally verified. Customer APIs and portability artifacts no longer expose private provider/model targeting. Agent admission now freezes the exact private execution target and immutable rate, reserves before provider contact, settles trusted normalized usage exactly once and releases failed or unstarted work without repricing retries. The handoff's earlier references to those foundations as wholly unencoded are superseded by the [UI revamp milestone report](ui-revamp-milestone-report.md). Token purchase/refund/promotion flows, commissioning, Account-credit settlement and subscription-remediation lifecycle remain open; the review record remains unsigned and every remote environment remains untouched.
...
- **Owner commercial policy is complete.** The local monthly Catalog, AI Token ledger and Agent-run admission/settlement boundaries are encoded. Private Stripe mapping, token commerce, commissioning, the account-credit-with-Support-check settlement implementation, legal review, privacy-controller facts and operational release fields remain implementation or external gates rather than unanswered product-policy choices.
