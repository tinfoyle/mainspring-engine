...
Implementation update (2026-08-26): the approved `$50`/no-Free launch Catalog identities, inactive zero-entitlement registration, configurable token Catalog, persistent AI Token ledger, paid-renewal grant projector, safe balance API and first-class provider-neutral Persona complexity contract are now implemented and locally verified. Customer APIs and portability artifacts no longer expose private provider/model targeting. The handoff's earlier references to those foundations as wholly unencoded are superseded by the [UI revamp milestone report](ui-revamp-milestone-report.md). Moving exact target/rate freezing and token reservation/settlement into Agent-run admission, token purchase/refund/promotion flows, commissioning, Account-credit settlement and subscription-remediation lifecycle remain open; the review record remains unsigned and every remote environment remains untouched.
