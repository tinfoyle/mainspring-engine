...
| `integration-connector-worker` | Recheck exact global placement/package authority, reconstruct one frozen cell delivery payload through its exact-version Marketing-only object reader, acquire one exact-operation credential lease, and invoke a closed capability connector | Serving traffic, browser/Agent identity, generic tools, reusable credentials, object listing/writes/deletes, arbitrary destinations; only the no-network mock adapter is currently composed and it is local-gated |
