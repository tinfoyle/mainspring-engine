...
| `mcp-gateway` | Authenticate audience/scope-bound MCP access tokens, recheck Account/package/placement authority, classify the exact tool and route the unchanged JSON-RPC body to its cell | Browser cookies, OAuth issuance, cell database access, provider credentials, arbitrary destinations |
| `tool-router` | Serve the app-router tool boundary over TLS 1.3, require an exact runner-broker workload identity, consume one-use tool context, and proxy a fresh least-authority cell request | Public ingress, runner/Kubernetes credentials, cell database access, provider credentials |
...
| `integration-connector-worker` | Lease and record content-free provider health for exact connection/credential bindings, recheck exact global placement/package authority, reconstruct one frozen cell delivery payload through its exact-version Marketing-only object reader, acquire one exact-operation mounted credential lease, and invoke the closed SMTP or create-only HTTPS publication adapter | Serving traffic, browser/Agent identity, generic tools, reusable credentials, object listing/writes/deletes, mutating health checks, redirects, private-address web destinations or automatic resend after uncertain SMTP delivery |
...
| `SPYGLASS_MCP_RESOURCE_ORIGIN` | Canonical public MCP audience, without a trailing slash |
...
The account API has no SMTP configuration. It serializes registration, invitation, credential-recovery, ownership-transfer, and verified-contact messages, encrypts each envelope with AES-256-GCM, and persists only ciphertext, a nonce, and key version in the durable outbox. Associated data binds the ciphertext to the outbox ID and notification kind. Plaintext tokens are never written to the outbox or application logs.
...
Verified-contact initiation is authenticated, exact-Origin, and passkey-protected. It queues encrypted messages to both the current and proposed mailbox while storing only a 32-byte token hash for 30 minutes. The current login remains unchanged until the new mailbox consumes the token. Completion atomically updates the User and local login identifier, advances the security version, revokes all sessions, consumes the challenge, queues completion messages to both addresses, and appends `primary_email_changed`; Account Memberships and entitlements are not rewritten.
...
|---|---|---|
| `SPYGLASS_ROUTE_ISSUER` | App router, MCP gateway, app API, route canary | Exact shared issuer name, normally `spyglass-app-router` |
| `SPYGLASS_ROUTE_SIGNING_KEY_ID` | App router, MCP gateway, route canary | Active or candidate non-secret key identifier |
| `SPYGLASS_ROUTE_SIGNING_KEY` | App router, MCP gateway, route canary | Standard Base64 encoding of exactly 32 random secret bytes |
...
| `SPYGLASS_ROUTE_CONTEXT_TTL` | App router, MCP gateway | Optional positive duration; defaults to `20s` and has a hard `30s` maximum |
...
| `SPYGLASS_DIRECTORY_CACHE_TTL` | App router, MCP gateway | Optional positive duration, at most `5m`; defaults to `30s` |
| `SPYGLASS_DIRECTORY_CACHE_CAPACITY` | App router, MCP gateway | Optional positive Account-route bound, at most 1,000,000; defaults to 10,000 |
...
| `SPYGLASS_MCP_RESOURCE_ORIGIN` | Account API, MCP gateway | Canonical public HTTPS MCP audience with no trailing slash |
| `SPYGLASS_MCP_RESOURCE_METADATA_URL` | MCP gateway, app API | Canonical public HTTPS RFC 9728 metadata URL |
| `SPYGLASS_MCP_AUTHORIZATION_SERVER` | MCP gateway | Exact application-origin OAuth issuer; one issuer is accepted |
| `SPYGLASS_MCP_TRUSTED_ORIGINS` | MCP gateway | Optional comma-separated exact browser origins allowed to send MCP requests with `Origin` |
| `SPYGLASS_ACCOUNT_EXPORT_DOWNLOAD_KEYS` | Account API, MCP gateway | Comma-separated `key-id=base64-key` HMAC keyring; every key is at least 32 bytes and rotation retains prior verifiers until all capabilities expire |
| `SPYGLASS_ACCOUNT_EXPORT_DOWNLOAD_ACTIVE_KEY_ID` | Account API, MCP gateway | Exact active key identifier present in the download keyring |
| `SPYGLASS_ACCOUNT_EXPORT_DOWNLOAD_CAPABILITY_LIFETIME` | Account API, MCP gateway | Optional positive duration no greater than `5m`; defaults to `2m` |
...
| `SPYGLASS_AGENT_EXECUTION_POLICIES_JSON` | Admission API | Complete private five-level complexity map to exact provider/model/fallback/reasoning targets; startup fails closed on omissions or unknown fields and accepted publications are refreshed prospectively |
...
The signing and verification keys follow the add-verifier, switch-signer, wait-for-expiry, remove-old-key sequence in [routing-boundary.md](routing-boundary.md). Tool-context keys follow the same overlap sequence but are a separate credential family and have a maximum 15-second lifetime. Account-export capability keys use the same overlap discipline but are a third credential family with a five-minute hard lifetime. The app-router database credential is global and needs read-only access to Account routing/current-entitlement state plus insert-only access to `tool_context_receipts`; it cannot read cell schemas. The MCP gateway role reads Account access/directory/security-posture and export-request state, inserts/updates export requests, inserts immutable export events, and executes the narrow digest-authentication function; it cannot read OAuth token tables, export events, artifact objects or any cell table. OAuth consent captures the approving session's passkey timestamp when it is current. Access-token rotation and refresh preserve that original timestamp and never advance it, so privileged export tools fail with `strong_authentication_required` after ten minutes. Tool receipts contain identifiers and digests only, are consumed before routing, and cascade away during Account erasure. The app-api credential is cell-local and cannot read global Users, Memberships, Entitlements, Billing, or sessions. Every routed cell mutation requires a UUID `Idempotency-Key`; transition and assignment require `If-Match`. Those semantic headers are included in the signed request binding.
...
Every HTTP service also exposes `GET /metrics` on its service port. The endpoint reports `spyglass_http_in_flight`, `spyglass_http_requests_total`, and a fixed cumulative `spyglass_http_request_duration_seconds` histogram with only static service names, a bounded method set, registered `ServeMux` route patterns, and response status classes. The reviewed duration buckets are 5, 10, 25, 50, 100, 250, and 500 milliseconds plus 1, 2.5, 5, and 10 seconds and positive infinity; they support mergeable p95 computation across replicas without customer-derived quantile labels. Unmatched routes collapse to `unmatched`, non-standard methods collapse to `OTHER`, and raw paths, Account IDs, request bodies, query strings, and headers are never labels. Monitoring must scrape this endpoint over the same trusted network boundary as the service; it is operational telemetry, not a public product endpoint.
...
| `SPYGLASS_TELNYX_API_KEY` | Telnyx API v2 bearer credential supplied only to the notification worker |
| `SPYGLASS_TELNYX_FROM` | Telnyx-approved originating number in E.164 format |
| `SPYGLASS_NOTIFICATION_POLL_INTERVAL` | Optional positive Go duration; defaults to `1s` |
| `SPYGLASS_HEALTH_ADDRESS` | Optional health listen address; defaults to `:8081` |

Delivery requires TLS 1.2 or newer. Workers claim one row with `FOR UPDATE SKIP LOCKED`, use a two-minute lease for crash recovery, refuse to send expired identity links and security codes, and retry delivery failures with bounded exponential backoff. The twelfth failed attempt enters terminal `dead_letter` state. Email uses SMTP. SMS uses the fixed Telnyx Messaging API v2 endpoint with an authenticated HTTPS POST containing only `to`, `from`, and the security-code text. A successfully accepted provider message is acknowledged as delivered; because delivery and PostgreSQL acknowledgement cannot share a transaction, a crash in between can produce a duplicate message. Identity links and six-digit codes remain single-use. Local and local-secure environments discard SMS delivery and return the development code only to the authenticated setup response; Stage and production never expose it.
...
## Baseline maintenance worker values

| Environment variable | Requirement |
|---|---|
| `SPYGLASS_GLOBAL_DATABASE_URL` | Required credential limited to workload Account/entitlement lookup and Work-capacity reservation |
| `SPYGLASS_CELL_DATABASE_URL` | Required credential limited to Baseline reads, Work creation and four maintenance queue functions |
| `SPYGLASS_CELL_ID` | Required exact cell placement identifier |
| `SPYGLASS_GLOBAL_MAX_DATABASE_CONNS` / `SPYGLASS_CELL_MAX_DATABASE_CONNS` | Optional positive pool caps; each defaults to `3` |
| `SPYGLASS_BASELINE_MAINTENANCE_POLL_INTERVAL` | Optional duration from `100ms` through `1m`; defaults to `1s` |
| `SPYGLASS_BASELINE_MAINTENANCE_LEASE` | Optional whole-second duration from `1s` through `30m`; defaults to `2m` |
| `SPYGLASS_BASELINE_MAINTENANCE_MAX_ATTEMPTS` | Optional integer from 1 through 100; defaults to `10` |
| `SPYGLASS_GLOBAL_ERASURE_CHECKPOINT_SEQUENCE` / `SPYGLASS_GLOBAL_ERASURE_CHECKPOINT_ROOT` | Required pinned global restore checkpoint |
| `SPYGLASS_CELL_ERASURE_CHECKPOINT_SEQUENCE` / `SPYGLASS_CELL_ERASURE_CHECKPOINT_ROOT` | Required pinned cell restore checkpoint |
| `SPYGLASS_HEALTH_ADDRESS` | Optional health listen address; defaults to `:8081` |

The technical queue contains only Account/assessment identifiers, assessment version, obligation instant and lease/error metadata. It is outside Account RLS solely for fair cross-Account claims and is inaccessible to the worker role except through claim, complete, fail and aggregate-stat functions. Baseline and Work rows remain forced-RLS. The named workload re-authorizes current package and placement state before every materialization, and can only create deterministic renewal/reassessment Work; all evidence and assessment decision commands remain human-only. `/health/status` exposes only counts, age and aggregate process outcomes.
...
## Prototype migration operator

`/prototype-transform` and `/prototype-import` are separate one-shot binaries in the admitted application image; they are not serving modes and are never started automatically. The importer requires dedicated `SPYGLASS_PROTOTYPE_IMPORT_GLOBAL_DATABASE_URL` and `SPYGLASS_PROTOTYPE_IMPORT_CELL_DATABASE_URL` credentials, exact `SPYGLASS_PROTOTYPE_IMPORT_CELL_ID`, the six `SPYGLASS_PROTOTYPE_IMPORT_OBJECT_*` values, and pinned `SPYGLASS_PROTOTYPE_IMPORT_GLOBAL_ERASURE_CHECKPOINT_*` plus `SPYGLASS_PROTOTYPE_IMPORT_CELL_ERASURE_CHECKPOINT_*` pairs. Sequence zero uses the canonical 64-zero root. Every other checkpoint must be copied from the environment's external erasure ledger evidence.

The global `spyglass_prototype_migration` role reads only Accounts, entitlement snapshots and the restore ledger. Its matching cell role receives only the proposed Knowledge/document migration tables, exact receipts/events, processed chunk reads and the restore ledger under forced Account RLS. A separate object credential may write/read/delete only exact source-object prefixes and read exact extracted-text prefixes. The operation rejects a destination whose current Account placement differs from the configured cell. Full procedure, retry semantics and certificate rules are in [Prototype Knowledge, document and Baseline migration](prototype-migration.md).
...
The Kubernetes reference uses these exact arguments and expects environment overlays to supply `spyglass-global-runtime`, `spyglass-cell-reference-runtime`, plus workload-specific `spyglass-account-api-secrets`, `spyglass-app-router-secrets`, `spyglass-mcp-gateway-secrets`, `spyglass-app-api-secrets`, `spyglass-admission-api-secrets`, `spyglass-route-receipt-worker-cell-reference-secrets`, `spyglass-billing-worker-secrets`, `spyglass-notification-worker-secrets`, `spyglass-entitlement-worker-secrets`, `spyglass-work-reconciler-secrets`, and `spyglass-baseline-maintenance-worker-cell-reference-secrets`. Those objects are intentionally absent from the repository. The account-api secret contains the exact-version download-reader identity, the export-capability keyring, and—when Google login is enabled—a projected `client-id:client-secret` file referenced by `SPYGLASS_GOOGLE_LOGIN_CLIENT_FILE`. The Google login client is distinct from every Integration OAuth client and is never delivered to a browser or another workload. The MCP gateway receives its constrained global credential, route-signing family, the same export-capability keyring and a client-only workload certificate, but no artifact-store credential. App-api receives only a cell credential and verification keyring. Admission-api receives only its narrow global usage credential and verification keyring. The route-receipt worker receives only its constrained cell cleanup credential. The entitlement worker secret needs only its constrained global-database credential. The Work reconciler and Baseline maintenance secrets contain distinct cell/global credentials and no serving, Stripe, SMTP or provider secret. No literal production credential belongs in source control or a rendered manifest.
...
### Optional Kimi model gateway provider

`SPYGLASS_KIMI_API_KEY`, `SPYGLASS_KIMI_ORIGIN` and `SPYGLASS_KIMI_MODEL_PRICING_JSON` enable the separate `kimi` provider in model-gateway. The origin defaults to `https://api.moonshot.ai`; the adapter appends `/v1/responses`. Stage passes these variables only to the model gateway. Empty key and price book leave Kimi disabled. Retain OpenAI for old admitted runs during a provider transition. Price books accept optional `cached_input_micros_per_million_tokens`; omission preserves legacy input accounting. See [Kimi integration and certification](kimi-provider.md) for K3 protocol differences, rollout and data-handling boundaries.
