...
AI Token commerce implementation checkpoint (2026-08-26): owner and billing-administrator one-time purchase now freezes the exact bundle code/version, Catalog version, USD amount and token quantity before leaving for Stripe. Signed paid Checkout projection validates its metadata against that local attempt and appends the frozen purchased grant once; final full-charge Refund projection appends one reversal of the surviving unused remainder and cannot rewrite settled usage. Promotion redemption requires the same recent-passkey billing authority, resolves only an effective immutable campaign, appends an expiring labeled grant and atomically enforces its per-Account and global issuance caps. A durable request lookup returns an exact prior redemption even after the campaign is removed from the current Catalog. Promotion issuance correlation is operational export state; the customer-safe grant and ledger evidence remain portable.
