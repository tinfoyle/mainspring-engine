...
Implementation checkpoint: the anonymous website renders the Infinite Ocean/Spyglass information architecture, proxies only the published Catalog through a bounded same-origin cache, and labels bundled fallback prices as illustrative. Public CTAs perform a GET-only handoff to the configured private application origin and never put names, email addresses, or business details in marketing-site requests. A selected paid offer remains an opaque intent: registration accepts it only when the current Catalog publishes an effective paid offer, the encrypted verification outbox and SMTP link preserve it, verification and sign-in revalidate it, and the authenticated billing view merely highlights it. Account creation remains free; only a separately authorized POST can create Stripe Checkout, and only projected signed Stripe state can change entitlements. Every public route now has one shared landmark/skip-link frame and passes the layout-independent axe-core WCAG 2.0/2.1/2.2 A/AA gate defined in [Accessibility target and release certification](accessibility-certification.md). Consent-aware analytics, real-browser/assistive-technology evidence, and deployed-origin link certification remain.

Private browser checkpoint: 20 representative identity, verified-contact, Account, security, Work, Agents, and lifecycle states now have a rendered-structure accessibility contract, including package-lock and read-only modes. Work interactions additionally have static contracts for labeled native assignment/reason dialogs, cancellation and success focus, atomic announcements, Account-keyed tab drafts, and removal of prompt-based commands. The local synthetic signup-to-owner-enrollment journey also reflows at 390 CSS pixels without horizontal overflow. Private real-browser axe, repeatable exact-artifact browser automation, zoom/contrast checks, and the assistive-technology matrix remain release evidence.
...
Implementation checkpoint: typed construction, lifecycle/role matrix, assignment and provenance values, maximum depth, optimistic versioning, private broker-backed active-item admission, routed create/transition/assignment/provenance/Conversation contracts, composite Account/Persona integrity, active published Persona assignment policy, immutable additive provenance policy, creation/lifecycle controls, assignment editing, explicit reason dialogs, Account-scoped tab drafts and live command announcements are implemented. See [work-module.md](work-module.md). Applied private-route browser and assistive-technology certification remains.
