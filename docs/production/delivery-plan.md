...
- Tenant mismatch and role/capability denial at every transport.
...
- PostgreSQL connection usage per tenant runtime and worker.
...
- `TenantContext` containing immutable tenant ID and safe request metadata.
- `Actor` containing actor kind, ID, tenant, roles, and authentication method.
...
- Dependency construction is not duplicated between tenant API and worker modes.
...
- Authentication, tenant boundary, request ID, rate limit, body size, CSRF, and security-header middleware.
...
- A synthetic request can be traced through gateway, API, workflow, activity, runner, and tool call without exposing content.

## 5. Phase 2 backlog — Work and Attention
...
### P2.2 Work persistence and queries
...
- Query plans meet agreed limits on representative tenant data.
- Repository errors are classified and no pgx types escape.

### P2.3 Agent-work dispatcher
...
### P2.5 Action ledger and executors
...
## 6. Phase 3 backlog — Knowledge and Baseline

### P3.1 Knowledge facts
...
- Retention, deletion, and tenant export behavior.
...
### P3.5 Evidence sources and planning
...
## 7. Phase 4 backlog — Workspace and Execution

### P4.1 Workspace domain and persistence
...
## 8. Phase 5 backlog — Remaining modules
...
## 9. Phase 6 backlog — Product surface

### P6.1 HTTP transport

- Feature route groups and generated request/response types.
- Consistent actor/tenant/role middleware.
...
- Bearer/workload authentication and tenant binding.
...
### P6.3 React application foundation
...
### P6.5 Legacy retirement

- Instrument compatibility route use by route and tenant cohort.
...
## 10. Phase 7 backlog — Operations and security
...
- Rate limits and abuse controls by edge, tenant, actor, capability, provider, and connector.
- Backup and point-in-time recovery for control and tenant databases.
- Object-store replication and restore.
- Tenant export and verified deletion.
- SLO dashboards and paging alerts.
- Run, action, migration, and connector operational consoles.
- Incident, restore, provider outage, compromised credential, and tenant-isolation runbooks.
- Penetration test and threat-model closure.
- Load, soak, chaos, and fleet-migration tests.
...
5. Enable production reads for internal tenants.
...
## 12. Cross-cutting acceptance matrix
...
- Is tenant identity verified at entry and repository boundaries?
...
## 13. Program completion checklist
