...
Implementation checkpoint: the runner gateway now requires distinct execute and side-effect-free reconcile methods for consequential definitions. A forced-RLS action authorization projection, ledger, attempt history and versioned executor/retry registry bind approval evidence to canonical input SHA-256, use the operation UUID as the stable provider idempotency key, serialize leases with cancellation, retry only frozen-policy definite no-effect failures, default uncertain errors to `unknown`, and force every expired/unknown/succeeded retry through reconciliation. The Attention-owned aggregate records and invalidates that exact projection atomically, including rollback on a conflicting runner binding. The first real Stripe Customer executor, dual-controlled manual resolution, content-redacted HTTP/MCP/Your Turn views, metrics, alerts and recovery runbook are implemented. Applied stage execution and final end-to-end journey certification remain before production acceptance can be claimed.
