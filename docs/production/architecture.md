...
Agents persists Account-owned Boardrooms, Persona identities, immutable PersonaVersions, Conversations, Runs, ordered RunPlan turns, Invocations, and Messages in separate forced-RLS tables. Every relationship and uniqueness constraint includes `account_id`. A separate content-free projection queue coordinates shared workers across Accounts. A worker must hold its exact expiring lease, authenticate and decrypt the Pod-bound envelope, and repeat domain validation before the accepted encrypted-exchange digest can be projected. Invocation success, the next conversation sequence, exactly one persona Message, terminal Run state, and projection settlement then commit atomically. Failure projection records no Message. Projection roles receive execute-only functions and no table reads; dead letters preserve ciphertext, and retention skips every unprojected Agent result. Account erasure cascades projection metadata and counts and removes every customer-owned Agents table before deleting the namespace.
