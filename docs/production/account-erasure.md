...
10. Account erasure never removes Memberships in other Accounts. After the Account is erased, a separate User-scoped workflow atomically rechecks all Memberships and active Affiliate relationships. If either exists, the identity, passkeys and sessions remain. If neither exists, the identity is orphaned and its live contact, credentials, sessions and identity data are erased; required restricted financial/security evidence is detached or minimized under its own retention rule.
