...
A reservation receipt records whether this call created the reservation or replayed an existing one. A finalized key cannot be resurrected. The service never releases an existing reservation after a payload conflict, because that reservation may belong to an already-created item. The cell admission adapter retries one connection failure with no HTTP response by repeating the identical proof and operation UUID; broker idempotency resolves a response lost after commit. It does not retry HTTP responses. A transport or commit error that remains after that budget has an unknown outcome: Spyglass retains capacity and returns `work_outcome_unknown`, and the caller repeats the exact body and Idempotency-Key. That retry either returns the idempotently created item or safely attempts the cell transaction again.
...
Completed technical queue rows are retained for 30 days by default, then deleted in bounded `FOR UPDATE SKIP LOCKED` batches through a partial `completed_at` index. Cleanup never targets live or dead-letter work. Operator events intentionally have no queue foreign key, so technical cleanup cannot erase recovery evidence.
...
Enabled Accounts now receive a prototype-informed New Work dialog, lifecycle controls and assignment editing for shared, self, external and active published Persona responsibility. Native modal command forms capture bounded reasons without browser prompts, return canceled focus to the initiating control, move successful focus to the updated detail and announce outcomes through an atomic live region. Create drafts use Account-keyed `sessionStorage`, survive same-tab Spyglass navigation, never cross Accounts or browser tabs, and are removed after successful creation. The browser generates and retains a UUID operation key for an exact command body, sends the current ETag on existing-item mutations, refreshes the queue/summary after success, and reloads a winning version after a precondition conflict. Read-only package access renders no mutation controls.
...
2. Certify the implemented dialogs, focus movement, live announcements and draft behavior on an exact connected artifact with private-route axe, keyboard-only and assistive-technology evidence.
3. Characterize and migrate prototype Work data Account by Account; verify numbers, hierarchy, state, assignment, provenance, and active-capacity reconciliation before switching traffic.
4. Add Attention concepts—human input, review, approval, and external action—as separate aggregates that reference Work rather than expanding Work into another catch-all store.
