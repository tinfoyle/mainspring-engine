...
1. Add ingress/router-replica, managed database, and sustained admission-outage exercises to the directory-routed, workload-authenticated boundary described in [routing-boundary.md](routing-boundary.md). Same-cell Service connection recovery, idempotent admission connection recovery, the two-cell placement/move/attack/outage contract, and candidate key/certificate canaries are implemented; admission API remains private.
