...
1. Add replica/database/admission failover exercises to the directory-routed, workload-authenticated boundary described in [routing-boundary.md](routing-boundary.md); the two-cell placement/move/attack/outage contract and candidate key/certificate canaries are implemented, and admission API remains private.
