...
- Status: Knowledge and document lifecycle complete; Baseline aggregate and persistence schema established, repository/application/surfaces next
- Phase: 3.2
...
Baseline-foundation checkpoint (2026-08-22): the pure aggregate now fixes the lifecycle as `interview -> inventory -> gap_review -> plan_approval -> active -> ready`, with reassessment archiving the explainable prior assessment and starting a new interview. Interview answers reference an exact accepted Fact revision or record an explicit reasoned unknown. Requirements freeze their evidence-catalog and scope-policy versions, responsibility and renewal policy; human evidence decisions bind immutable Knowledge Evidence identities. Every requirement must be explicitly satisfied, a gap or not applicable before a plan can be submitted. Owner/Administrator approval binds the exact plan ID, content digest and pre-plan assessment version, and readiness refuses unresolved gaps. Six normalized cell tables carry these records under forced Account RLS, exact Knowledge Fact/Evidence and Persona references, immutable decision/event history, movement fencing and exact erasure/restore accounting. Domain replay and full `ubunturojo` Docker tests are green. Classified persistence, application authorization/idempotency, deterministic Work-plan creation and HTTP/MCP/private surfaces remain open.
