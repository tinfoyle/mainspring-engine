...
- Status: Knowledge/document lifecycle and Baseline lifecycle complete through governed interview/scope catalogs, exact accepted-Fact/evidence resolution, scoped source grants, linked-Work evidence confirmation, renewal/reassessment Work scheduling, HTTP/MCP and deterministic materialization; prototype transformation and reconciliation remain
- Phase: 3.2
...
Source-grant and evidence-selection checkpoint (2026-08-22): after plan approval, an Owner or Administrator may create a narrow, immutable, read-only email or Google Drive grant tied to the exact Account, assessment and connector identity. Email grants freeze one to twenty folders and optional UTC date bounds; Drive grants freeze one to fifty selected folders and reject date scope. Expanding access requires a new grant, while revocation is optimistic, reasoned and permanent. Two forced-RLS tables retain the grant and content-redacted lifecycle events with Account movement fencing, exact erasure counts and stable pagination; no credentials, folder names or connector content enter events. Three generated HTTP operations and two MCP tools use the Integrations package, while the Catalog now makes Knowledge an explicit Integrations dependency. The app router now has an exact allowlist for every Baseline route—repairing the previously missing browser-to-cell route—and separates Knowledge from Integrations authority. Evidence decisions resolve the exact Account-owned Knowledge Evidence identity before mutation; human callers may register only owner statements, and Agent derivations or not-yet-bound integration records cannot satisfy a requirement. Connector adapters will bind captures to these grants during P3.4; until then the boundary fails closed. The following completion/maintenance checkpoint closes the linked-Work and renewal portion; prototype transformation remains open P3.2 work.
