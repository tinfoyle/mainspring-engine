...
func TestPostgresGlobalErasureIsCrossStoreExactAndIdempotent(t *testing.T) {
	...
	if _, err := owner.Exec(ctx, `CREATE ROLE `+globalFunctionRole+` NOLOGIN NOBYPASSRLS;
		...
		GRANT SELECT,DELETE,UPDATE ON spyglass.account_namespaces,spyglass.account_audit_events,spyglass.work_item_number_counters,
			...
			spyglass.agent_dispatch_queue,spyglass.agent_queue_operator_events TO `+cellFunctionRole+`;
		...
		ALTER FUNCTION public.spyglass_erase_account_cell_without_runner_actions(uuid,uuid,bigint,bytea,bigint,bigint,text,bytea,bytea,timestamptz) OWNER TO `+cellFunctionRole+`;
		ALTER FUNCTION public.spyglass_erase_account_cell_without_agents(uuid,uuid,bigint,bytea,bigint,bigint,text,bytea,bytea,timestamptz) OWNER TO `+cellFunctionRole+`;
		ALTER FUNCTION public.spyglass_erase_account_cell_without_agent_projection(uuid,uuid,bigint,bytea,bigint,bigint,text,bytea,bytea,timestamptz) OWNER TO `+cellFunctionRole+`;
